...
pub(crate) async fn tool_agents_list(_args: &Value) -> Result<String> {
    ...
    for (i, agent) in agents.iter().enumerate() {
        ...
        if agent.has_agent_md { config_parts.push("agent.md"); }
        if agent.has_persona { config_parts.push("persona"); }
        if agent.has_tools_guide { config_parts.push("tools.md"); }

        output.push_str(&format!(
            "   Config: {} | Memories: {}\n",
            if config_parts.is_empty() { "none".to_string() } else { config_parts.join(", ") },
            agent.memory_count,
        ));
    }

...
use crate::cron;
use crate::globals::{
    ACP_MANAGER, APP_HANDLE, APP_LOGGER, CHANNEL_DB, CHANNEL_REGISTRY, CRON_DB, MEMORY_BACKEND,
...
pub(crate) fn init_app_state(initial_store: ProviderStore) -> AppState {
    ...
    // Initialize ACP control plane
    {
        ...
        if store.acp_control.enabled {
            ...
            tokio::spawn(async move {
                acp_control::registry::auto_discover_and_register(&registry_clone, &acp_config)
                    .await;
            });

...
两道闸都过不去的功能，只需要跟随一条**可配置的全局模型链（带跨模型降级）**，不需要 Agent 那套模型链、能力开关、记忆配置。Agent Team、Subagent 等确需完整 Agent 配置的功能不在本文档范围，见 [Agent 配置与解析链](agent-config.md) / [子 Agent 系统](subagent.md)。

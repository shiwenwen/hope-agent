...
> 返回 [文档索引](../README.md) | 关联：[MCP 客户端](mcp.md) · [工具系统](tool-system.md) · [配置系统](config-system.md) · [权限系统](permission-system.md)
...
审批裁决的收口是 [`permission::engine::resolve_async`](../../crates/ha-core/src/permission/engine.rs)：它先跑同步的 `resolve` 拿到决策，再按需接 smart-judge 的 LLM 复裁（完整优先级与 `SessionMode` 三档 `Default` / `Smart` / `Yolo` 语义见 [permission-system](permission-system.md)）。[`tools/execution.rs::resolve_tool_permission`](../../crates/ha-core/src/tools/execution.rs) 是构造 `ResolveContext` 并调 `resolve_async` 的唯一入口：它把 `is_dangerous_skip_active()` 的结果喂进 `ResolveContext.global_yolo`，**YOLO bypass 落在同步 `resolve` 里**——它见 `ctx.global_yolo || session_mode == Yolo` 即整体跳过审批门，返回 `Allow`。

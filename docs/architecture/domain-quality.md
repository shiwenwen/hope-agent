...
固有 `impl` 不能跨 crate，所以上浮到 ha-improve 的方法以自由函数 `fn f(db: &SessionDB, …)` 的形式存在（如 `run_domain_quality_for_session`）。ha-improve 的生产代码对数据库零直接触点。新增 owner 入口时遵循同一分工：**SQL 写台账，编排写机器**。ha-improve 的分层背景见 [前后端分离架构](backend-separation.md)。
...
- [dashboard](dashboard.md)：`domainQuality` 趋势区块的完整口径。
- [backend-separation](backend-separation.md)：机器 / 台账分层的通用规则。

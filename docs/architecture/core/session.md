...
自动记忆提取也只读取侧聊自己的消息：阈值/空闲提取使用最近 6 条非快照用户与助手消息；压缩前提取使用最近最多 64 条，再应用原有字符预算。两者都经 `SessionDB::load_side_chat_memory_history` 读取持久账本，不使用可能混入父会话内容的原生上下文或压缩摘要；尚无自身消息时不调用提取模型。普通会话保持原有上下文取材路径，侧聊正常对话仍使用完整继承上下文。

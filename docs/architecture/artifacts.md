...
`artifact.json` / `artifact_version_meta.payload_json` 是 analysis 版本的不可变真相源；项目目录中的 `index.html` 是 renderer-owned 可重建投影。读取或恢复旧 Analysis Artifact 时，Core 可以原子重建当前预览以获得新版响应式/可访问性修复，但不得修改版本号、canonical hash、来源或 evidence。`canvas_versions.html` 只保留历史快照，不升级为真相源。
...
- Analysis/Markdown 由确定性 Rust renderer 生成语义 HTML；无 CDN。Analysis 阅读器采用 30 秒决策层、证据层、审计层：页头/结论/发现卡片，静态 HTML/CSS bar 或 SVG line 图，显式 presentation table，建议/限制，以及可展开的质量和 claim validation。
- Analysis 的 `tables[].columns/rows` 优先于绑定 dataset 的全量列；显式空数组表示不展示，禁止回退原始 dataset。数值转换使用 `columnFormats` 的明确 unit/scale，禁止根据列名猜测百分比；dataset 仍是计算/追溯依据。所有宽表只允许在局部 `.table-scroll` 中横向滚动，文档根禁止横向溢出，避免 Canvas resize 后保留 body scrollLeft 导致左侧裁切。

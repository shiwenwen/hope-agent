...
- **参考图 → 匹配产物（「照着这张图做」，真多模态）**：`CreateArtifactInput.reference_image_b64`（非媒体形态）在 `generate_design_artifact` 里经 `extract::prepare_reference_image`（大小闸 + 魔数嗅探 + `downscale_for_vision` 降采样重编码）规整成**视觉附件**，与文字要求一起经 `run_vision_streaming` 上行——**选中的视觉模型直接看原图**流式生成（旧「describe 成文字 brief 再生成」两阶段已退役：文字中转丢精确配色 / 布局比例 / 字体质感）。入口：首页卡片传图（`+` / 粘贴 / 拖拽）、项目内「从参考图生成」弹窗、品牌包（每件产物都带原图，N 件 = N 次带图调用）。图-only（无文字 brief）用固定复刻指令；prompt 尾部附 `REFERENCE_IMAGE_GUIDANCE` 复刻指引，system 用 `VISION_UNTRUSTED_SYSTEM` 信封（图内文字 = 复刻素材，绝不作指令）。前端 canvas 先降采样（≤1600px JPEG）再上传（HTTP body 不超限）；参考图规整失败**回退文本 brief / 空壳**，不阻断。区别于 §6.4 反向提取（图→设计系统 token）：这里图→**可交付产物**。
...
- **前端 `LaunchHome`（prompt-first 首屏）**：大标题 + 大输入框（Cmd/Ctrl+Enter 生成）+ 形态 chip + **模板快选行**（`list_design_recipes_cmd` 拉内置 recipe 目录，点选 → 填入该形态 + 场景 brief，可编辑后生成）+ 内联设计系统选择器 + **参考图（`+` 按钮 / 粘贴 / 拖拽收单张，缩略预览 + 移除；有图无文也可生成）** + **生成模型 chip**（通用 `ModelSelector`；传图态 `requireVision` 置灰非视觉模型；选择记 `DesignConfig.last_model`、随项目创建写 `DesignProject.default_model`）+ 生成 / 品牌包按钮。**传图瞬间当前模型不认图 → 自动切到可用视觉模型 + toast（删图不切回，模型选择粘性）；无任何视觉模型 → 传图入口拦截并提示去设置**。`generateFromHome` = 建项目 → 带 prompt（+ 参考图 + modelOverride）建产物（后端生成）→ 打开；产物创建失败**回滚删除刚建的孤儿项目**；生成中禁用最近项目磁贴防导航被完成回调劫持。
...
- **解析单一入口 `service::resolve_code_dir(project)`**（code_dir > ha_project_id > None）；任一源失效（目录被删 / HA 项目被删）**fail-safe 按未绑定处理**、不自动清列，GUI 经 `get_project_code_binding` 的 `stale` 标记标红（对齐 cron delivery target stale 先例）。互斥由 `service::set_project_code_binding` 单点校验（双源同传 bail；db 层 `set_project_code_binding` verbatim 覆写非 COALESCE）。

**绑定生效面（四处）**：① 反向提取对话框 codebase 通道预填生效目录；② **agent 提取读根扩张**——`scoped_local_path` 的允许根从「会话工作目录 ∪ 附件」扩为「∪ 绑定仓库」（经 `session_bound_code_dir`：会话 → 设计线程锚 → 项目 → `resolve_code_dir`，只读不建项目），fail-closed 语义不变；③ **设计线程会话 working_dir 对齐**——`mark_session_as_design_thread` 锚定新线程时写入生效目录，`set_project_code_binding` 变更时 best-effort 对齐该项目全部既有线程（解绑清空）——设计对话里的 agent 由此能用 `read`/`exec` 真读仓库（受既有权限引擎管辖）；④ `DesignCodeBinding` token 同步目标目录预填。
...
**页面组织（一个项目 = 一个应用，每个产物 = 一个页面）**：产物顶部横向标签条 + 单一稳定预览之外，加一个 studio 内「所有页面」缩略图总览（`LayoutGrid` 切换，复用 `ArtifactThumb` 懒挂载沙箱缩略图；`component` kind 客户端水合、`sandbox=""` 禁脚本 → 用类型图标占位）。页面**双击标签就地改名**（轻量 `rename_design_artifact_cmd`：仅改 `title`、不重渲染不新增版本）、**复制页面**（`duplicate_design_artifact_cmd`：同项目深拷贝 + 版本行，持 `artifact_lock` 且拒绝拷 `generating`）、**拖动排序**（`design_artifacts.position` 列，`reorder_design_artifacts_cmd` 按序写 position；`list_artifacts` 改 `ORDER BY position ASC`；`create_artifact` 位序自增追加）。项目名在工作室标题**点击就地改**（复用 `update_design_project_cmd` 的 title，乐观同步 `activeProject`）。

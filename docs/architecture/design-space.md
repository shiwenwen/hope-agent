...
- **工作室（`DesignView` 项目态）**——左对话 / 右预览（对齐 open-design 的 chat-to-edit 布局，「对话改写」是头号迭代路径）：
...
- **批注两条出口**：批注卡「带到对话」经面板 `DesignChatPanelHandle.addQuote` 把反馈作 quote chip 塞进 composer（`DesignView` 持 `chatPanelRef`，展开被折叠的对话栏）——用户可补充后随 turn 发、AI 在完整上下文下迭代（对齐 open-design 的批注→composer）；「一键精修」仍走 `design_comment_refine_cmd` 单条快捷精修。

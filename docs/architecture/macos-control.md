...
> 返回 [文档索引](../README.md)
>
> 状态：桌面 bridge、权限 readiness、snapshot/wait、apps/windows/act/menu/clipboard/dialog、display/window 截图镜像与审批分类已接入
...
| 路径 | 职责 |
| --- | --- |
| `crates/ha-core/src/mac_control.rs` | 公共类型、bridge 注册、status/permissions/snapshot/wait/apps/windows/act/menu/clipboard/dialog/capture_frame 入口、snapshot cache、截图文件 LRU、错误统计 |
...
```text
status -> snapshot/wait -> apps/windows/act/menu/clipboard/dialog -> snapshot 验证
```

...
| HTTP 路由 | `crates/ha-server/src/lib.rs` 的 `.route(...)` | **369** |
| 前端 COMMAND_MAP | `src/lib/transport-http.ts::COMMAND_MAP` | **361** |
...
| 分类 | 数量 | 说明 |
|---|---|---|
| ✅ 两端完全对齐（在 COMMAND_MAP 中） | 361 | 常规请求/响应命令 |
...
三端差集稳定在 5 条合法非 REST 命令（4 条 Desktop-only 权限 + `save_avatar` multipart）。新增 Tauri 命令时须同步补 HTTP 路由 + COMMAND_MAP，保持该差集不变——详见下文"新增接口 checklist"与"验证脚本"两节。
...
`create_session_cmd` 与 `chat` 在自动创建新会话时都支持可选 `incognito: boolean`，返回的 `SessionMeta` 也会包含 `incognito` 字段；当请求同时带了 `project_id` 时 `incognito` 被强制为 `false`（互斥）。`list_sessions_cmd` / `search_sessions_cmd` / `list_project_sessions_cmd` 接受可选 `active_session_id` 参数：默认会过滤掉所有 incognito 会话，`active_session_id` 让正在打开的那个无痕会话仍出现在 sidebar / 搜索结果里。`purge_session_if_incognito` 在前端 `handleSwitchSession / handleNewChat / handleNewChatInProject` 切走当前 session 之前调用，仅当目标 session 当前为 incognito 时硬删，否则 no-op。

`set_session_working_dir` 接受 `{ workingDir: string | null }`，后端 `canonicalize` 路径并校验是否为存在的目录，返回 `{ updated: true, workingDir: <canonical> }`；`null` 或空串清除选择。该字段以 `SessionMeta.workingDir` 呈现，被 `system_prompt::build` 注入到 "# Working Directory" 段落（位于 Project / Project Files 之后、Memory 之前）。与 Project / Incognito 正交：三者可同时启用。在 HTTP 模式下前端没有原生目录选择器，改走 `GET /api/filesystem/list-dir`（见 Filesystem 域）的服务端目录浏览器。
...
### Filesystem（仅 HTTP 模式）

| HTTP | 说明 |
|---|---|
| `GET /api/filesystem/list-dir?path=<abs>` | 列出服务器本地目录，供前端的 HTTP 模式目录浏览器驱动 `set_session_working_dir`。Tauri 直接用原生 `@tauri-apps/plugin-dialog` 选目录，因此不暴露为 Tauri 命令。参数要求绝对路径，后端会 canonicalize 并校验 `is_dir`；无 `path` 参数时返回平台默认根（Unix: `/`，Windows: `USERPROFILE`）。响应 `{ path, parent, entries: [{ name, isDir, isSymlink, size, modifiedMs }] }`，按目录优先 + 名字升序排序。 |
...
截至 2026-04-21 已经把所有原缺口逐条补齐：§7.1 的 14 条 `No REST mapping` 风险命令全部登记进 COMMAND_MAP，§7.2 的 9 条缺 HTTP 路由里 7 条补上了 `ha-server` handler + 路由，1 条（`list_builtin_tools`）原本就已经存在只是清单写错已修正，1 条（`load_session_messages_cmd`）前端无引用已从 `invoke_handler!` 清理。当前三端差集稳定在 §7.3（Desktop-only）+ `save_avatar` multipart。
...
# 期望：5 行（4 条 Desktop-only 权限 + save_avatar）
```

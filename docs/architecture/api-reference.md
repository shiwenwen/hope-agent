...
| 源 | 位置 | 数量 |
|---|---|---|
| Tauri 命令 | `src-tauri/src/lib.rs` 的 `tauri::generate_handler!` | **678** |
| HTTP 路由 | `crates/ha-server/src/lib.rs` 的 `.route(...)` | **640** |
| 前端 COMMAND_MAP | `src/lib/transport-http.ts::COMMAND_MAP` | **665** |
...
| 分类 | 数量 | 说明 |
|---|---|---|
| ✅ 两端完全对齐（在 COMMAND_MAP 中） | 665 | 常规请求/响应命令（所有 COMMAND_MAP 条目都有 Tauri 命令对应） |
...
```bash
# 1. Tauri 命令总数（截至 2026-07-01：678）
...
# 2. HTTP 路由总数（截至 2026-07-01：640）
grep -cE '^[[:space:]]+\.route\(' crates/ha-server/src/lib.rs

# 3. COMMAND_MAP 条目数（截至 2026-07-01：665，不含闭合 `}` 的行）

...
| 源 | 位置 | 数量 |
|---|---|---|
| Tauri 命令 | `src-tauri/src/lib.rs` 的 `tauri::generate_handler!` | **365** |
| HTTP 路由 | `crates/ha-server/src/lib.rs` 的 `.route(...)` | **368** |
| 前端 COMMAND_MAP | `src/lib/transport-http.ts::COMMAND_MAP` | **360** |
...
| 分类 | 数量 | 说明 |
|---|---|---|
| ✅ 两端完全对齐（在 COMMAND_MAP 中） | 360 | 常规请求/响应命令 |
...
| `get_server_runtime_status` | `GET /api/server/status` | ✅ (免鉴权) — 返回 `{ boundAddr, startedAt, uptimeSecs, startupError, eventsWsCount, chatWsCount, activeChatStreams, activeChatCounts: { desktop, http, channel, total } }`；`activeChatStreams` 现在等于 `activeChatCounts.total`（在跑的 `run_chat_engine` 数量），不再是 WS 订阅者数 |

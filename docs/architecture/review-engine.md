...
- 相邻子系统：[goal](goal.md) · [workflow](workflow.md) · [lsp](lsp.md) · [session](session.md) · [automation-model](automation-model.md) · [coding-improvement-loop](coding-improvement-loop.md)
...
请求体字段（`RunReviewInput`）：`scope`（默认 `local`）、`baseRef`（当前非空即拒）、`goalId`、`profiles[]`、`focusPaths[]`、`ideContext`。Tauri / HTTP 命令增删须同步 [api-reference](api-reference.md)。

...
- 只支持 `scope=local` 的未提交改动审查；非空 `baseRef` / `profiles` 会被拒绝，避免字段被误认为已经生效。
...
- `workflow.review({ focusPaths?, baseRef?, profiles? })` 复用同一 durable review API，默认审查 local diff，并自动继承当前 workflow run 的 `goal_id`。

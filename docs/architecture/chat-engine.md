...
- 相关子系统：[failover](./failover.md) · [context-compact](./context-compact.md) · [session](./session.md) · [memory](./memory.md)
...
Chat Engine 内置完整的模型降级和重试逻辑。退避基数 / 上限 / 单模型重试次数统一来自 `failover::FailoverPolicy::chat_engine_default()`（详见 [failover.md](./failover.md)）：
...
| **Failover** | `classify_error()` + `FailoverPolicy::chat_engine_default()` | 错误分类和退避计算，见 [failover.md](./failover.md) |
...
| **Plan Mode** | `plan_context_override` | 透传到 Agent 限制工具和路径，见 [plan-mode.md](./plan-mode.md) |

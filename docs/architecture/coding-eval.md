...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 4.1 已实现。本文只记录已经落地的自动化评测层；人工 gold task 体系仍见 [Coding Eval 体系方案](../roadmap/coding-eval.md)。
...
| `improvement` checks | trend scope、failure category、proposal kind/status、eval success rate、repair loop blocked、proposal apply status、artifact target 断言。 |

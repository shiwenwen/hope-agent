...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 3.8 已实现。本文只记录已经落地的自动化评测层；人工 gold task 体系仍见 [Coding Eval 体系方案](../roadmap/coding-eval.md)。
...
它不是完整的端到端 agent benchmark，也不调用 LLM。Phase 3.7 先把“可确定性回归”的能力钉住；Phase 3.8 继续把 workflow 内的 review / verification host API 纳入同一套回归；Phase 3.9 把 bounded repair loop 的停机与证据链纳入回归：
...
| `crates/ha-core/tests/fixtures/coding_eval/*.json` | Phase 3.7/3.8/3.9 控制面 fixture。 |

...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 5.7 已实现。本文只记录已经落地的自动化评测层；人工 gold task 体系仍见 [Coding Eval 体系方案](../roadmap/coding-eval.md)。
...
| `baselineKind` | 基线类型，默认归一为 `deterministic_mock`；外部真实模型基线传 `external_model`。 |
...
默认执行模式是 `fixture_patch`，因此不会访问外部模型；它验证的是 task schema、候选 diff、Review / Verification / Context / Goal / Task scorer 的端到端胶水。Phase 5.7 会把每次 pack run 作为可审计历史保存；`baselineKind` 必须标清 deterministic / mock / external，Dashboard 不把确定性替身数字冒充成真实模型能力。Phase 5.6 已补上不访问外部服务的 mock tool-call 基线；外部真实模型成功率的大规模基线仍属于后续阶段。
...
这层不会把 proposal 自动写进项目规则或 skill；它只验证 `coding_improvement` 聚合器是否能稳定消费 durable control-plane 数据。Phase 4.2 允许 fixture 显式声明 `promoteAppliedProposal`，用于验证 promotion 路径本身，但仍然是 owner-plane 的确定性动作，不会由 proposal generation 或 apply 隐式触发。Phase 5.1-5.7 的 task-level report / execution metrics / pack history / strategy effect history 会进入 Improvement Loop 与 Dashboard，因此可以把任务级失败、执行失败、tool-call 缺失、scope creep 或策略回归变成可审计趋势。
...
- 在 release gate 中消费持久化 pack / strategy history，形成可配置的发布质量阈值。

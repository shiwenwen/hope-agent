...
| 设计系统（DESIGN 契约 9 段） | `DESIGN.md` 9 段 + token 编译 | 原创原型语言 + 品牌风格参考 | P2 |
| 内置设计系统库（大量品牌） | 6 套原创原型 + 一批品牌风格参考（种子展开、附免责声明）+ 用户自建/提取 | 全量级对标 + 本地提取护城河 | P2 |
...
| DQ6 | 内置设计系统 | **6 套原创原型语言 + 一批品牌风格参考（附免责声明）** | 用户拍板全量级对标：与对标开源项目同为开源/本地/社区，法律位置一致，指名性合理使用；品牌参考为对公开视觉语言的独立再诠释，渲染必附免责声明、非官方 |

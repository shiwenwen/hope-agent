...
> OpenComputer 多渠道即时通讯接入 — Rust 原生实现
...
IM Channel 系统是 OpenComputer 的多渠道即时通讯接入层，允许用户通过 Telegram、Discord、Slack 等 IM 平台与 AI Agent 对话。系统基于 Rust 原生实现，充分利用 tokio 异步运行时获得优秀的性能和较低的资源开销。
...
`supports_media: Vec<MediaType>` 决定 dispatcher 是否会把模型生成的图 / 音视频 / 文件以原生消息形式投递给该渠道。`Vec::new()` 时统一降级为"贴下载链接的纯文本兜底"（[`build_media_fallback_lines`](../../crates/oc-core/src/channel/worker/dispatcher.rs)）。
...
补齐参考实现：Telegram 走 SDK 内置 `InputFile`（[telegram/media.rs](../../crates/oc-core/src/channel/telegram/media.rs)），Discord 走单 POST multipart（[discord/media.rs](../../crates/oc-core/src/channel/discord/media.rs)），飞书走"上传 → 引用 key 发消息"两步（[feishu/media.rs](../../crates/oc-core/src/channel/feishu/media.rs)），WeChat 走"获取 CDN 上传 URL → AES 加密上传 → 引用消息项"自建加密链路（[wechat/media.rs](../../crates/oc-core/src/channel/wechat/media.rs)）。新增渠道时按平台 API 形态选最接近的范本套用 — 大多数 IM 平台属于 Discord 或飞书两类。
...
- **大小上限**：`MAX_DISCORD_FILE_BYTES = 25 MiB`（[discord/media.rs](../../crates/oc-core/src/channel/discord/media.rs)），超限返回 Err 让 dispatcher 走"下载链接文本"兜底
- **caption 处理**：`payload.text` 与每个 `OutboundMedia.caption` 在 [`merge_captions`](../../crates/oc-core/src/channel/discord/media.rs) 里合成单段 Discord `content`，避免拆条
- **MIME 推断**：[channel/media_helpers.rs](../../crates/oc-core/src/channel/media_helpers.rs) 优先 URL Content-Type，回退按文件扩展名查内置表
...
`MediaType` → 飞书 `file_type` 映射（[feishu/media.rs](../../crates/oc-core/src/channel/feishu/media.rs)）：

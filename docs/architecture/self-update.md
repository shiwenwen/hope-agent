...
- **sibling swap 语义**：`self_contained::install` 主二进制 swap + 冷烟自检通过后，把归档内 `extra_binaries` 声明的附带可执行文件（当前即 `ha-browser-host`）逐个 `atomic_replace_binary` 到主二进制同目录——**best-effort**：单个 sibling 失败只 `app_warn` 不阻断也不回滚主升级（host/core 版本错配由 broker 版本握手显式诊断）；自检失败回滚只还原主二进制、不动 sibling（swap sibling 发生在自检之后，回滚路径上 sibling 尚未被碰过）。旧 manifest 无 `extra_binaries` 字段 → serde 默认空数组，行为与从前一致。

...
| `sessions.db` | `chat_turns.ui_surface`、turn 状态与消息边界，`sessions.last_read_message_id`，以及可跨重启恢复的 pending `ask_user_question` groups；`ui_surface` 是 additive migration，历史 NULL 永不按 source 猜测，索引随 migration 一次性建立 |
...
activity 查询时才把进程内 approval registry 与 SQLite pending Ask groups 聚合为当前交互数和最早倒计时；不能把这个聚合结果回写数据库或据此假设 approval 可跨重启恢复。`.install-*` staging 超过 24 小时才清理；`.trash` 最长保留 7 天且最多 256 项。清理、导入、删除、恢复与选择校验共享跨进程库锁，任何缓存或 renderer state 都不能成为包、配置、activity 或未读的第二真相源。

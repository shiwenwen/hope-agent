...
> 新增于 Phase 7.6，Phase 7.7 增加人工校准动作，Phase 7.10 增加隔离的 Smoke Run Center。Dashboard Learning Tab 的通用领域质量区块；前端直接调用 `evaluate_domain_quality_gate`、`list_domain_eval_runs`、`list_domain_eval_tasks`、`list_domain_eval_fixture_runs` 与 `record_domain_eval_calibration`，后端事实源见 [Domain Eval 与 Quality Gate 控制平面](domain-eval.md)。
...
- Dashboard 默认只读历史，不生成 proposal、不运行 eval、不触发连接器动作；唯一写动作是用户点击「Mark reviewed」时记录 `domain_eval_calibrations` 人工复核。`trace_fixture` / `agent` fixture runner 仅作为 owner API / 测试能力保留，不直接挂在质量门按钮上；合成样本通过 `SessionKind::EvalFixture`、`sourceType=fixture_*` 与 `domain_eval_fixture_runs` 隔离展示，避免污染真实质量判断。

...
| `operation` | 按 `model_usage_events.operation`（purpose 标签，见 [`automation-model`](automation-model.md) §2.5）精确匹配。**无下拉框**，只能点击「Token 用量趋势」里 operation 明细表的行下钻写入，与 `model_id` 的下钻方式一致 |
...
**发布与消费分层**：`emit` 与 9 个事件常量定义在内核 [`learning_events.rs`](../../crates/ha-core/src/learning_events.rs)——生产者遍布内核 / skills / knowledge / ha-mcp 四层，发布面若留在 dashboard 会让它们全部反向依赖 ha-dash。`dashboard/learning.rs` 只做只读聚合，并把 `emit` / `EVT_*` 按原路径再导出，老调用点不受影响。DDL / INSERT / prune / 会话级联删除都在 `SessionDB`。详见 [backend-separation](backend-separation.md)。

### 事件常量（9 个）

| 类别 | 常量 | 触发点 |
|------|------|----------|
| Skill 生命周期 | `EVT_SKILL_CREATED` / `_PATCHED` / `_ACTIVATED` / `_DISCARDED` / `_USED` | `skills::author` CRUD + 激活 / 丢弃，见 [skill-system](skill-system.md) |
| 记忆召回 | `EVT_RECALL_HIT` / `EVT_RECALL_SUMMARY_USED` | 召回命中 + 召回摘要被注入 system prompt，见 [memory](memory.md) |
...
命令与端点全表见 [api-reference](api-reference.md)。

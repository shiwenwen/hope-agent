...
- **模型解耦**：分析用哪个模型与主对话 Agent 完全独立配置，走后台自动化调用链解析（详见 [模型 vs Agent 统一配置](automation-model.md)）。
...
Tauri 命令与 HTTP 端点的增删须同步 [api-reference](api-reference.md)。
...
解析结果是一条 `Arc<Vec<ActiveModel>>`，贯穿本次报告的每一次独立 LLM 调用（facet 提取 + 章节生成）；每次调用各自经 `automation::run` 走真跨模型降级（构造失败或调用失败都 `continue` 下一候选），不共享单个 Agent 的失败状态。报告里记的 `analysis_model` 标签命名的是**整条链**（形如 `主模型 (+N fallbacks)`）——因为每个事实都可能实际由某个后备产出，只标 `chain[0]` 会声称一份链本身不保证的确定性。详见 [模型 vs Agent 统一配置](automation-model.md)。

...
| `record_domain_evidence` | 从 Context Retrieval 候选行的“证据”按钮写入当前 session evidence，并刷新通用任务工作台。 |
...
- 工作台只聚合 owner-plane 读模型和已有显式动作按钮；写路径仅限用户显式点击候选行「证据」或「转任务」后记录当前 session evidence / task，不自动创建 WorkflowRun、不访问连接器、不发送/分享/导出内容。
...
- Workspace Context 区块展示 domain profile、access issue 与 domain action chips；“复制引用”、“加入证据”和“转任务”已落地，其余动作先以 `metadata.domainActions` 暴露给后续 owner action。
...
Context Retrieval 查询本身仍是只读 owner-plane 查询，不创建 workflow run、不写 evidence、不访问连接器；只有用户在 GUI 候选行显式点击「证据」或「转任务」时，才会分别调用 owner action 写入当前 session evidence / task。

...
- Goal detail 会把 `sourceType=domain_evidence` 的证据单独分到「领域证据」区块，展示 source、confidence、access scope 与 workflow run/op provenance，避免非 coding 证据淹没在 validation / diff / task 证据里。

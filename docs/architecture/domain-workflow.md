...
Workspace 的 Workflow section “新建工作流”表单已接入 domain workflow template：
...
Phase 8.4 在 `src/components/chat/workspace/WorkspacePanel.tsx` 新增「通用任务工作台」区块。当前 Workspace 信息架构将它收在 Advanced Diagnostics 下，位于「推荐上下文」之后、「LSP / Review / Verification / 领域复核」之前。它是 GUI 聚合层，不新增后端表，也不改变任何执行/授权语义。
...
「真实样本验收」卡片是 Workspace 内的只读派生视图，不新增后端表：它从当前 session 的 domain evidence、Artifact Export Guard、Connector Action Guard、Operational Gate 与 Soak Report 计算覆盖领域数、控制面记录数、已排空 Workflow / Loop / Campaign 样本、Connector E2E evidence 数，以及事故、缺证据、缺排空、样本陈旧、输出预算耗尽、缺连接器 E2E、单领域覆盖等缺口。它回答“当前会话是否已经有真实场景样本可供验收”，不启动采样、不调用连接器、不修改 gate 结果；卡片会显示验收快照 ID、证据等级、来源分布、控制面组成、保守验收进度（必需项通过数 / 必需项总数 + 百分比，failed gate 会拉低必需项进度，守门通过要求至少有已观察 gate 且全部 passed，跨领域覆盖作为额外加分），并把领域样本、证据链、排空样本、样本新鲜、跨天覆盖、预算健康、事故清零、守门通过、连接器 E2E（仅涉及外部动作时）逐项显示为通过 / 待补 / 阻塞，让用户能直观看到“还差多少”和“差在哪里”。卡片还派生只读「验收结论」和「证据等级」：验收结论为 `不可验收` / `待补样本` / `可局部复核` / `可验收`，由 danger gap、未通过必需项、warning/info gap 和控制面样本数保守计算；证据等级为 `未采样` / `阻塞样本` / `局部样本` / `局部验收` / `非外部动作候选` / `真实 E2E 候选`，明确这份材料只能用于采样待办、问题定位、局部复核、非外部动作验收，还是已经包含连接器执行/复核 evidence 可进入最终人工复核。来源分布按 evidence 的可见 `sourceMetadata` / evidence type / access scope 统计 workflow、connector、fixture/mock、manual、public 和 restricted 数量；控制面组成按 Soak / Operational summary 显示 workflow、loop、campaign 与 connector E2E 的完成/总量，只作为 reviewer 判断样本成分的提示，不替代权限、gate 或真实 evidence。验收结论、证据等级、来源分布和控制面组成进入 UI、采样清单和复制报告，帮助人工 / Claude Code review 直接判断这份样本能不能作为最终验收证据；它们不是新后端 gate，不写库、不改变运行状态。卡片还派生「验收矩阵」，把 Workflow、Loop、Campaign、连接器 E2E、跨领域覆盖拆成独立样本跑道，逐项显示当前 evidence 数字和下一步动作；未完成跑道可显式「转任务」，矩阵用于产品验收和 review packet，不改变 readiness 百分比，也不把未涉及外部动作的会话强行判 failed。“样本新鲜”使用 Soak Report 的 `latestActivityAgeSecs` 做 GUI 侧保守验收，并读取后端 `sampleDays / requiredSampleDays` 展示跨天覆盖：多天 Soak 窗口默认至少要求 2 个不同自然日的真实活动样本，单日样本或最近活动超过 24 小时时 Soak Report 只能保持 `insufficient_data`，不会被当成长期稳定性通过；1 天窗口只要求 1 天样本，避免短窗口验收被不可能的跨天要求卡住。“预算健康”使用 Soak Report 的 `workflowBudgetExhaustedEvents` 和最大 output-token 用量：出现预算耗尽时会压低验收进度、显示阻塞并可转任务，提醒用户先收窄上下文、拆分阶段或减少无效输出后再跑最小验证样本；它同样不自动改预算、不自动重跑 workflow。验收缺口会按 `阻塞` / `待补` / `扩展` 排序和标记，critical soak incident、连接器执行后复核缺失、跨天样本不足、样本陈旧、预算耗尽、failed E2E gate 这类会先展示，避免用户把长任务事故或性能/成本失控误当成普通补样本。卡片可一键复制 Markdown 验收报告，内容包含当前指标、必需项、验收结论、证据等级、来源分布、控制面组成、审计索引、复核协议、验收矩阵、每条跑道的证据 checklist、缺口、守门状态、最近 evidence provenance、长跑审计事故 / timeline 和推荐下一步，供人工 / Claude Code / PR review 复核；审计索引先给出 `acc-xxxxxxxx` 形式的快照 ID，再集中列出领域、控制面样本、样本新鲜度、跨天覆盖、连接器执行/复核数、预算、Gate 快照、Evidence IDs 和 Soak 窗口，方便 reviewer 对齐报告与当前工作台状态。卡片、复制报告和「采样清单」任务使用同一个 review context 生成快照 ID，避免同一轮验收材料出现两个 ID。该快照 ID 是基于报告摘要、gate 状态、evidence id、来源分布、控制面组成与 soak 计数的非安全确定性指纹，用于比较两份验收材料是否来自同一批可见状态，不替代真实 evidence 或完整性签名。复核协议明确只有 `可验收` 才能作为最终验收证据、转任务/按钮点击不能替代真实 evidence、长任务必须复核 Gate / Soak、连接器 E2E 必须来自测试账号或沙箱数据并包含执行/复核/回滚说明。复制动作只读，不创建 evidence、不修改 gate，且不展开完整 `sourceMetadata` 或大段私密正文。每个未通过必需项、未完成样本跑道与验收缺口都可由用户显式「转任务」；只要存在验收缺口、未通过必需项或非 optional 的样本跑道未完成，即使只有一个扩展缺口，也可一键创建「采样清单」任务，清单会写入当前指标、验收结论、证据等级、来源分布、控制面组成、审计索引、复核协议、验收进度、最近样本年龄、跨天覆盖、连接器执行/复核数、输出预算状态、必需项逐项状态、验收矩阵、每条跑道的证据 checklist / 刷新目标、带严重度标签的验收缺口和采样动作，均复用 `create_session_task` 落入 TaskProgressPanel。
...
- 「长跑审计」卡片调用 `generate_domain_soak_report({ sessionId, windowDays: 14, maxItems: 8 })`，显示样本数、跨天覆盖、连接器执行后复核数、critical/warning incidents、最长 workflow drain、已闭环/未闭环审批等待、owner control intervention、恢复次数、output-token budget 消耗/耗尽信号、最近 timeline、Soak Report recommended next steps 和最近事故建议，并可复制 Soak Markdown 报告用于复盘 / review；推荐建议与事故行的「转任务」按钮复用 `create_session_task`，把连接器执行后复核缺失、跨天样本不足、样本陈旧、未排空、审批悬空或具体事故处理建议落成可跟踪 task。
...
- 通用任务工作台读取 Artifact Export Guard、Connector Action Guard、Operational Gate 与 Soak Report state；当这些守门非通过时，相关卡片在 Advanced Diagnostics 中展示交付守门、外部动作守门、运行稳定性和长跑审计证据。定位入口只做滚动/展开，不自动刷新、不自动重试、不自动修改 gate 结果。

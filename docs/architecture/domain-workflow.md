...
- 「长跑审计」卡片调用 `generate_domain_soak_report({ sessionId, windowDays: 14, maxItems: 8 })`，显示样本数、跨天覆盖、critical/warning incidents、最长 workflow drain、已闭环/未闭环审批等待、owner control intervention、恢复次数、output-token budget 消耗/耗尽信号、最近 timeline、Soak Report recommended next steps 和最近事故建议，并可复制 Soak Markdown 报告用于复盘 / review；推荐建议与事故行的「转任务」按钮复用 `create_session_task`，把跨天样本不足、样本陈旧、未排空、审批悬空或具体事故处理建议落成可跟踪 task。

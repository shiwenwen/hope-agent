...
- 「运行稳定性」卡片调用 `evaluate_domain_operational_gate({ sessionId, windowDays: 14 })`，显示 workflow 完成/活跃、loop 成功/失败、campaign 活跃和阻塞 check；失败 / 样本不足 check 行可显式「转任务」，只创建可跟踪 task，不自动 retry、cancel、approve 或修改 gate 结果。
- 「长跑审计」卡片调用 `generate_domain_soak_report({ sessionId, windowDays: 14, maxItems: 8 })`，显示样本数、critical/warning incidents、最长 workflow drain、审批等待、恢复次数、最近 timeline、Soak Report recommended next steps 和最近事故建议，并可复制 Soak Markdown 报告用于复盘 / review；推荐建议与事故行的「转任务」按钮复用 `create_session_task`，把样本不足、未排空、审批悬空或具体事故处理建议落成可跟踪 task。

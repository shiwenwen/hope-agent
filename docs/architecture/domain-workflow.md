...
- 工作台只聚合 owner-plane 读模型和已有显式动作按钮；唯一写路径是用户显式点击候选行「证据」后调用 `record_domain_evidence` 记录当前 session evidence，不自动创建 WorkflowRun、不访问连接器、不发送/分享/导出内容。
...
- Workspace Context 区块展示 domain profile、access issue 与 domain action chips；“复制引用”和“加入证据”已落地，其余动作先以 `metadata.domainActions` 暴露给后续 owner action。

...
- 只读 op：`status`、`permissions`、`snapshot`、`elements.find`、`wait`、`apps.list/frontmost/installed/search`、`dock.list`、`spaces.list`、`windows.list`、`act.dry_run`、`menu.list`、`dialog.inspect`。
- 普通突变 op 进入审批：`apps.activate/launch`、`dock.launch/hide/show`、`spaces.switch/move_window`、`windows.focus/move/resize/minimize`、除 `dry_run` 外的 `act.*`、`menu.click`、`dialog.dismiss`。

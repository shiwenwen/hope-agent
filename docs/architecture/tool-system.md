...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力的只读骨架。当前支持 `action=status|permissions|snapshot`；`snapshot` 是 Phase 2A 的 AX-only 前台 App / 窗口 / 元素摘要，截图帧、点击、输入、窗口、App、菜单等能力留到后续阶段。桌面 Tauri 注册 bridge，server/headless/HTTP 返回 `supported=false`。 |

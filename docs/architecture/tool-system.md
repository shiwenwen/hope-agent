...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力。当前支持 `action=status|permissions|snapshot|wait|apps|windows|act|menu|dialog`；`snapshot` 返回前台 App / 窗口 / AX 元素摘要，`includeScreenshot=true` 会落盘主显示器 JPEG 并打开右侧镜像，`wait` 支持 `present|gone` 轮询 AX snapshot。`apps` 支持 list/frontmost/installed/search/activate/launch/quit；`windows` 支持 list/focus/move/resize/minimize/close；`act` 支持 click/click_point/double_click/right_click/type/set_value/hotkey/scroll/drag；`menu` 支持 list/click；`dialog` 支持 inspect/accept/dismiss。桌面 Tauri 注册 bridge，server/headless/HTTP 返回 `supported=false`。 |

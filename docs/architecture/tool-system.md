...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力。当前支持 `action=status|permissions|snapshot|wait|apps`；`snapshot` 返回前台 App / 窗口 / AX 元素摘要，`includeScreenshot=true` 会落盘主显示器 JPEG 并打开右侧镜像，`wait` 轮询只读 AX snapshot 直到 app/window/element query 命中，`apps` 支持 running app list/frontmost/activate。`apps.activate` 只切换焦点且进入审批；点击、输入、窗口、菜单、launch/quit/hide 等能力留到后续阶段。桌面 Tauri 注册 bridge，server/headless/HTTP 返回 `supported=false`。 |

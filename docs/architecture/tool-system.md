...
记忆工具（`save_memory`, `recall_memory`, `update_memory`, `delete_memory`, `memory_get`, `update_core_memory`, `project_memory`）由 agent 级 `memory.enabled` 与全局 `memoryExtract.enabled` 共同控制——开启时进入 eager/deferred 决策，关闭时全部 `Hidden`。`project_memory` 还要求当前会话绑定有效 Project；无痕会话全部隐藏且执行层再次拒绝。UI 不显示这些工具的单独开关，记忆能力作为整体管理。
...
均为 internal（永不审批）。除 `update_core_memory` / `project_memory` 使用本机 Markdown 外，其余在 SQLite + FTS5 + 向量检索后端上操作。
...
| `update_core_memory` | deferred, internal | `core_memory` 的兼容别名，更新常驻系统提示的 `MEMORY.md`。`action`：`append` / `replace`；`scope`：`global` / `agent`（默认 `agent`）。 |
| `project_memory` | deferred, internal | 项目文件式自动记忆。`MEMORY.md` 只保存 XML-escaped 有界索引并进入稳定 prompt；`list / search / read` 按需发现详情（read 默认 12k 字符、可分块并返回 raw-file BLAKE3），已有主题 `write / delete` 必须带 `expectedFileHash`。mutation 由项目级 OS 锁串行化并原子更新主题与索引。仅项目会话 eligible。 |

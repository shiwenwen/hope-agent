...
`deferredTools.enabled` 是 opt-in 开关（默认 **false**）。关闭时全部 ~52 个工具 schema 一次性发送给 LLM，单轮 system prompt + tools 基线可达 **16K+ token**；开启后只发送 ~15 个 core 工具 schema（外加 `skill` / `runtime_cancel` 等 always_load 豁免项），其余靠 `tool_search` 元工具按需发现，基线可压到 **5–6K token**。

### Core（always_load，始终发送 schema）

15 个 CORE 工具 + `skill` / `runtime_cancel` 通过 `always_load = true` 单独豁免，不在 `CORE_TOOL_NAMES` 名单里也照样进入 core schema。常量定义：[`tools/definitions/types.rs::CORE_TOOL_NAMES`](../../crates/ha-core/src/tools/definitions/types.rs#L120-L136)；豁免逻辑见 [`registry.rs::get_core_tools`](../../crates/ha-core/src/tools/definitions/registry.rs#L127-L132)（过滤条件 `!t.deferred || t.always_load`）。
...
| 运行时控制 | `runtime_cancel`（内置 `always_load = true` + `internal = true`，async_job / subagent / process / cron 统一取消入口） |
...
| MCP | `mcp_resource`（list / read MCP server 资源）, `mcp_prompt`（list / get MCP prompt 模板） |
...
| `acp_spawn` | `subagent_tool_enabled()`（与 `subagent` 同闸；按需在分支后独立 push） |

...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力。当前支持 `action=status|permissions|snapshot|wait|apps|windows|act|menu`；`snapshot` 返回前台 App / 窗口 / AX 元素摘要，`includeScreenshot=true` 会落盘主显示器 JPEG 并打开右侧镜像，`wait` 轮询 AX snapshot 直到 app/window/element query 命中。`apps` 支持 running app list/frontmost/activate/launch；`windows` 支持 list/focus/move/resize/minimize；`act` 支持 click/type/set_value/hotkey/scroll；`menu` 支持 list/click。突变操作进入审批；close/quit/hide/delete/force quit 等破坏性动作仍不可用。桌面 Tauri 注册 bridge，server/headless/HTTP 返回 `supported=false`。 |

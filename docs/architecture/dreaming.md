...
> 本文覆盖 Dreaming 独有的固化 / claim / 注入 / 纠错 / 评测链路。记忆召回引擎、Embedding 配置、自动提取等记忆系统的通用机制见 [`memory.md`](memory.md)；Tauri 命令 ↔ HTTP 路由的完整签名对照见 [`api-reference.md`](api-reference.md)。
...
面向用户本人的命令（Tauri ↔ HTTP 一一对应，完整签名与语义见 [`api-reference.md`](api-reference.md)）：

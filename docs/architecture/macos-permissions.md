...
| [`platform/mod.rs`](../../crates/ha-base/src/platform/mod.rs) | facade（`pub(crate)`）：`system_permissions_supported` / `system_permissions_platform_name` / `check_system_permission_item` / `request_system_permission_item`，把上层 `permissions.rs` 与平台 `imp` 解耦 |

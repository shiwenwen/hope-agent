...
- Domain 候选的 `metadata.domainActions` 声明可引用、可补 evidence、可摘要、可请求用户确认、可标记冲突、可转 task 等 owner action 能力；GUI 已提供“复制引用”和“加入证据”的真实轻量动作，其余动作先结构化暴露给后续执行入口。
...
- 继续扩展 `domainActions` 真实 owner action：生成摘要、发起 ask_user、标记冲突、转成 task。候选创建 evidence 已在 Workspace Context 行落地。

...
> 返回 [技术文档索引](../README.md)
>
> Context Retrieval 是工作台里的「推荐上下文与行动入口」：把散落在会话各处的信号聚合成一个有优先级的候选列表。推荐本身只读、不创建新的 durable run；点击候选行上的按钮才会触发聚焦审查 / 验证等真实动作。它既服务编程场景，也服务通用（非编程）场景。

## 1. 目标

Context Retrieval 回答一个用户视角的问题：
...
它最初为编程场景设计，现在同样服务通用场景：编程侧的 Git / LSP / 审查 / 验证排序保持原样，领域工作流只在识别到 domain run、domain evidence、显式 domain 或可推断的 Goal 时才加权。
...
- 最近修改高于最近读取。
- 文件搜索和 LSP symbol 只在 query 非空时参与。
...
- 文件搜索只有 query 非空才运行，继续受目录遍历上限约束。
...
- `domainActions` 的轻量 owner action 已覆盖生成摘要、请求用户确认、创建 evidence、标记冲突与转 task；后续重点转向更多连接器只读候选和召回质量指标。
- 接入真实连接器只读候选（Gmail / Calendar / Drive / Sheets）时必须继续走 access issue + 授权边界，不伪造缺失来源。

Workflow 内产生的信号也会进入候选：`workflow.review()` / `workflow.verify()` 复用同一套 focused owner API，其产出的审查发现、验证步骤与 Goal evidence 都会出现在 Context Retrieval 里。

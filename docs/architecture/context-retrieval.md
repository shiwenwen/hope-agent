...
```rust
context_retrieval_for_session(db, session_id, ContextRetrievalInput { query, limit, ide_context, domain, template_id })
```
...
- Domain context 是可选增强：显式 `domain/templateId` 优先，其次从 `workflow_runs.kind = domain:<domain>`、`domain_evidence_items.domain`、Goal objective / criteria 轻量推断。没有 domain 信号时不影响 coding context。
...
- Domain 候选的 `metadata.domainActions` 声明可引用、可摘要、可补 evidence、可请求用户确认、可标记冲突、可转 task 等后续 owner action 能力；GUI 已提供“复制引用”的真实轻量动作，其余动作先结构化暴露给后续执行入口。
...
```text
get_context_retrieval(sessionId, query?, limit?, ideContext?, domain?, templateId?)
```

HTTP：

```text
GET /api/sessions/{sid}/context-retrieval?query=<q>&limit=<n>&domain=<domain>&templateId=<id>
...
- 将 `domainActions` 接到真实 owner action：从候选创建 evidence、生成摘要、发起 ask_user、标记冲突、转成 task。

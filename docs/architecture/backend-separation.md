...
三种模式共享 `ha_core::init_runtime()` 这一个全局单例 setter；后台任务按模式选 `start_background_tasks`（桌面 + server）或 `start_minimal_background_tasks`（acp）。
...
sequenceDiagram
    ...
    Note over Init: 全部 OnceLock<br/>(SessionDB / LogDB / MemoryDB<br/>Logger / CronDB / Channel Plugins<br/>SubagentCancels / ACP Manager / EventBus)
    ...
    Note over BG: channel approval/ask_user listener<br/>cron scheduler<br/>channel auto-start<br/>dreaming idle-trigger<br/>3 个 daily retention loops<br/>MCP init + watchdog<br/>ACP backend auto-discover
    Setup-->>Tauri: Ok(())
...
sequenceDiagram
    ...
    Main->>Core: init_runtime()
    Note over Core: 与桌面同一组 OnceLock<br/>EventBus 由 init_runtime 兜底创建
    ...
    Note over Core: cron scheduler / channel listeners /<br/>retention loops / dreaming / MCP+watchdog
...
sequenceDiagram
    ...
    Main->>Core: init_runtime()
    Note over Core: 与桌面同一组 OnceLock
    Main->>BgRt: Builder::new_multi_thread(2 worker, "acp-bg")
    Main->>BgRt: bg_rt.spawn(start_minimal_background_tasks)
    Note over BgRt: 仅 channel listeners +<br/>ask_user 一次性清理 +<br/>async_jobs replay + MCP init_global
...
三种模式都调用 `ha_core::init_runtime()`，所有 OnceLock 在三种模式下都被 populate。`init_runtime` 拆出来的 `build_app_state()` 仅桌面用（构造 Tauri `AppState`），server / ACP 直接消费 OnceLock。后台任务的差异详见 [process-model.md §跨模式能力对照](process-model.md#跨模式能力对照)：server 与桌面对齐跑全套 `start_background_tasks`，ACP 跑 `start_minimal_background_tasks`（按设计跳过 daily 循环、cron、channel auto-start、dreaming、MCP watchdog、ACP backend auto-discover）。

历史 gap：在 fix/server-acp-runtime-init 之前，`hope-agent server start` 只手写 SessionDB / ProjectDB / EventBus 三个 OnceLock，`hope-agent acp` 只开 SessionDB；任何依赖其它 OnceLock 的工具（`recall_memory` / `manage_cron` / `subagent` 等）在 daemon 模式下都会 `"XXX not initialized"`。修复方案的拆分思路记录在 [`crates/ha-core/src/app_init.rs`](../../crates/ha-core/src/app_init.rs) 顶部注释。

...
装配层通过 `pub use` 把契约层子模块再导出，因此 `slash_commands::{types,parser,registry,fuzzy}::…` 这些历史路径依然可用。详见 [backend-separation](backend-separation.md)。
...
| `/loop` | 无参 / `<duration> <prompt>` / `<prompt> every <duration>` / `<prompt>` / `[every\|until\|status\|pause\|resume\|stop]` | 创建或控制当前会话 durable loop；无参创建维护型 Loop 并读取可选 `loop.md`，无间隔的 prompt-only 写法创建自定节奏 Loop，创建型命令后端立即触发第一轮（详见 [loop](loop.md)） | `DisplayOnly` |
...
| `/kb` | `[on\|off]` 可选 | **IM 专用**：群聊内 per-chat 确认 / 关闭当前 chat 的 KB 访问（需账号级 `kbAccessOptIn` 已在桌面设置开启）；DM 仅报状态。详见 [knowledge-base](knowledge-base.md) | `DisplayOnly` |
...
| `smart` | 放行工具自报「高置信度」的调用，必要时跑 judge model 二次确认（详见 [permission-system](permission-system.md)） |
...
前端把技能命令的 `PassThrough` 通过 `handleSend(expandedMessage, { displayText: commandText })` 送出：用户气泡显示原始 `/skillname args` 并落库，模型收到的是内联了 SKILL.md 的展开消息。详见 [技能系统 §斜杠命令的 Inline 内联路径](skill-system.md)。
...
`/workflow` 首先是当前会话的 **Workflow Mode 开关**：开启后模型会看到 `workflow` 控制工具，并在任何适合动态编排的任务里自行判断是否创建 durable workflow run。它不是 coding-only，也不要求先进入 coding 模式。除此之外，`/workflow` 还是 workflow run 的轻量命令面，适合在聊天里快速查看长任务状态；完整创建 / 预览 / 审批与控制中心 UI 走 [workflow](workflow.md) 的 owner API / Workspace 面板。
...
`/goal` 是当前会话 active Goal 的轻量控制面；完整状态、证据指标和操作按钮在 [goal](goal.md) 的 Workspace Goal section 中展示。
...
详见 [behavior-awareness](behavior-awareness.md)。
...
| `/kb [on\|off]` | `ChannelAccountConfig.settings.kbAccessChats` | 群内 per-chat 确认 KB 访问，需账号级 `kbAccessOptIn` 开启；查不到 / 不匹配 fail closed。详见 [knowledge-base](knowledge-base.md) |

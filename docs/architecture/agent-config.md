...
Memory UX v2 的 Fast Recall 由全局 `memory.recall` 控制，默认启用且不调用额外 LLM。`ActiveMemoryConfig.enabled` 保留为旧配置兼容入口：开启时等价于为该 Agent opt-in Deep Recall；`include_claims` 继续控制旧 Active Memory 候选是否包含 effective-active claims。两字段仍 per-agent 存在 `agent.json`、不进 `ha-settings`，普通用户通过 Agent 记忆总开关和全局“相关时想起 / 深度召回”界面配置。记忆侧契约详见 [memory.md](memory.md) / [dreaming.md](dreaming.md)。
...
3. 经 `CoreMemoryRepository` 注入全局 `~/.hope-agent/memory/MEMORY.md` 到 `global_memory_md`

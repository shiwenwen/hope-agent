...
- **子 Agent 模式**（`plan_subagent: true`）：独立子 Agent 负责探索代码和制定计划，主 Agent 上下文不被规划阶段的探索细节占用，执行阶段更干净

两种模式共用同一套 5 阶段规划流程（深度探索→需求澄清→架构设计→计划编写→审查优化）和以文件为中心的富细节计划格式。用户可在 Review 阶段审查计划、请求修改、浏览版本历史。批准后进入 Executing 阶段，实时追踪步骤进度，支持暂停/恢复、执行中修改计划、Git Checkpoint 一键回滚。
...
graph TB
    ...
    subgraph 后端
        ...
        TQ[tools/plan_question.rs<br/>交互问答 + session 路由]
...
#### Planning 阶段：5 阶段规划流程 + 以文件为中心的计划格式
...
- Consider edge cases, error handling, trade-offs

## Phase 4: Plan Composition
- Use submit_plan tool to submit the final plan
- Plan must be implementation-ready, structured by files
...
### Step N: <verb> — <file_path>
  - Code blocks for struct/type definitions, function signatures
  - Tables for mappings
  - file_path:line references for existing code to reuse
...
**文件**：`src-tauri/src/subagent/spawn.rs`、`src-tauri/src/plan.rs`
...
```
用户消息 → chat 命令 → plan_subagent=true?
  → YES: spawn_plan_subagent() → 子 Agent 独立探索/提问/提交计划
         → plan_question/submit_plan 事件路由到父 session（PLAN_SUBAGENT_SESSIONS 注册表）
         → skip_parent_injection=true（结果不注入父对话）
         → 用户后续消息通过 SUBAGENT_MAILBOX steer 转发
  → NO:  主 Agent 内联执行（PlanAgent 模式，上下文连续）
```
...
| `get_plan_owner_session_id()` | plan_question/submit_plan 用此函数路由事件到父 session |
...
| 规划流程 | 5 阶段（Explore→Plan→Review→Write→Exit） | 5 阶段（复刻 Claude Code） | **5 阶段 + 结构化问答 + 以文件为中心的计划格式** |
| 计划格式 | ≤40 行精简（文件+改动） | 增量写入 plan 文件 | **以文件为中心，含代码块/结构体/函数签名/表格** |
| Plan 展示 | Plan 文件 + 编辑器 | 终端文本 | **PlanCardBlock + PlanPanel 双视图** |
| 交互问答 | AskUserQuestion | 普通 question 工具 | **plan_question 类型化选项 + UI 卡片** |
...
| 文件 | 用途 |
|------|------|
| `src-tauri/src/plan.rs` | 核心：六态状态机 + 文件 I/O + 解析 + 常量 + 路径权限 + Git Checkpoint + 版本管理 + 问答注册 + 子 Agent 注册表 + spawn_plan_subagent |
...
| `src-tauri/src/subagent/spawn.rs` | 子 Agent 计划模式限制继承 + plan_agent_mode 支持 + skip_parent_injection |
| `src-tauri/src/subagent/types.rs` | SpawnParams 扩展（plan_agent_mode / plan_mode_allow_paths / skip_parent_injection / extra_system_context） |
...
| 文件 | 改动点 |
|------|--------|
| `src-tauri/src/lib.rs` | `mod plan` + 注册 12 个命令（含 cancel_plan_subagent） |
...
| `src-tauri/src/provider.rs` | ProviderStore 新增 `plan_subagent: bool` 全局设置 |

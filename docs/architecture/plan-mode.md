...
Plan Mode 是 Hope Agent 的可视化交互计划模式，实现六态状态机驱动的完整规划-审查-执行工作流。
...
| **Plan File** | Markdown 格式的计划文件，项目本地存储于 `.hope-agent/plans/` |
...
| Plan 文件 | `.hope-agent/plans/*.md` | 计划内容持久化 + 版本历史 |

### 工具限制与路径权限

> 详见 [工具系统架构](tool-system.md) 了解完整的工具权限模型。

**文件**：`src-tauri/src/commands/chat.rs`（桌面）/ `crates/ha-server/src/routes/chat.rs`（HTTP）→ 调用 `crates/ha-core/src/plan/`
...
- 允许：`.hope-agent/plans/` 目录下的 `.md` 文件
- 允许：`~/.hope-agent/plans/` 全局目录下的 `.md` 文件
- 拒绝：所有其他路径
...
## Restrictions
- CANNOT modify files outside .hope-agent/plans/
...
**项目本地化存储**：优先存储在项目本地 `.hope-agent/plans/`，非 git 仓库回退到全局 `~/.hope-agent/plans/`。
...
**文件**：`crates/ha-core/src/tools/submit_plan.rs`
...
**文件**：`crates/ha-core/src/tools/plan_step.rs`
...
**文件**：`crates/ha-core/src/tools/amend_plan.rs`
...
sequenceDiagram
    ...
    Backend->>Git: git branch hope-agent/checkpoint-{id}-{ts}
...
**分支命名**：`hope-agent/checkpoint-{short_session_id}-{timestamp}`
...
**文件**：`crates/ha-core/src/subagent/spawn.rs`、`crates/ha-core/src/plan/`
...
**文件**：`crates/ha-core/src/slash_commands/handlers/plan.rs`
...
sequenceDiagram
    ...
    Backend-->>Panel: "hope-agent/checkpoint-xxx"
...
- Planning/Review 状态下 `write`/`edit` 工具仅允许操作 `.hope-agent/plans/` 目录下的 `.md` 文件
...
| 特性 | Claude Code | OpenCode | Hope Agent |
|------|-------------|----------|--------------|
...
| `crates/ha-core/src/tools/plan_step.rs` | update_plan_step 步骤进度工具 |
| `crates/ha-core/src/tools/amend_plan.rs` | amend_plan 执行中修改工具 |
| `crates/ha-core/src/subagent/spawn.rs` | 子 Agent 计划模式限制继承 + plan_agent_mode 支持 + skip_parent_injection |
| `crates/ha-core/src/subagent/types.rs` | SpawnParams 扩展（plan_agent_mode / plan_mode_allow_paths / skip_parent_injection / extra_system_context） |
...
| `crates/ha-core/src/session/types.rs` | `SessionMeta.plan_mode` 字段 |
| `src-tauri/src/commands/chat.rs` | 六态分支：双模式分发（plan_subagent 设置）+ 工具限制 + 路径权限 + 模型覆盖 + 提示词注入 |
| `crates/ha-core/src/provider/` | AppConfig 新增 `plan_subagent: bool` 全局设置 |
| `crates/ha-core/src/agent/mod.rs` | plan_ask_tools + plan_executing + plan_tools_enabled + plan_mode_allow_paths |
| `crates/ha-core/src/agent/providers/*.rs` | 消费 `agent/mod.rs` 已注入/已过滤的 plan 工具 schema 并发起 API 请求 |
| `crates/ha-core/src/tools/mod.rs` | 4 个 plan 工具模块 + 常量 |
| `crates/ha-core/src/tools/execution.rs` | 4 个工具 dispatch |
| `crates/ha-core/src/tools/definitions.rs` | 4 个工具定义 |
| `crates/ha-core/src/slash_commands/registry.rs` | 注册 /plan 命令 |
| `crates/ha-core/src/slash_commands/types.rs` | 6 个 CommandAction 变体 |

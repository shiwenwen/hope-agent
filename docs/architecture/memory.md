...
## 确定性评测（Golden Fixtures，下一代 Dreaming）

下一代 Dreaming 靠离线 eval 守红线、不靠感觉（设计 §9）。评测分三层避免 LLM 非确定性让 CI 变脆：
...
| Human review set | 真实样本匿名化后人工标注 precision/recall | release 前抽样 |

本期落地 **deterministic 层**：

- `memory/dreaming/eval.rs`——fixture 类型 + `load_fixtures()`（读 `tests/fixtures/dreaming/*.json`）+ `evaluate(backend, fixture)`。`evaluate` 经**公共 API 跑真实读路径**（`write_claim_candidate` 播种 → `list_claims` / `get_claim` / `load_prompt_candidates_with_project` / `evidence_quote` 断言），不重写被测逻辑。
- `crates/ha-core/tests/fixtures/dreaming/*.json`——7 个 fixture：`user_preferences`（基础 + 证据覆盖）/ `project_scope_isolation`（作用域泄漏）/ `temporal_supersede`（effective-expired 抑制）/ `conflict_resolution`（needs_review 不进注入）/ `incognito_exclusion`（证据展开 fail-closed）/ `source_evidence`（active claim ≥1 证据）/ `legacy_sync`（dead claim 隐藏 linked managed memory）。每个 fixture 限定**唯一 scope 命名空间**，故共享一个 DB 也互不串扰。
- `tests/dreaming_eval.rs`——集成测试（独立进程，独占 claim store global），逐 fixture 调 `evaluate` 并断言所有 check 通过（当前 7 fixture / 29 check）。
- **契约**：改动 claim 读路径 / effective-status / hidden-set / scope 过滤 / evidence 授权等安全红线时，须在 fixtures 加 case 或保既有绿。`valid_until` 用 `"past"` / `"future"` token 解析为固定远端日期，使 CI 与时钟无关。

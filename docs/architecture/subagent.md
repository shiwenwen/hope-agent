...
| 文件 | 行数 | 职责 |
|------|------|------|
| `src-tauri/src/subagent/mod.rs` | 79 | 模块入口、常量（DEFAULT_MAX_DEPTH/MAX_CONCURRENT_PER_SESSION 等）、7 个全局 LazyLock 静态量、re-exports |
| `src-tauri/src/subagent/types.rs` | 145 | SubagentRun / SpawnParams / SubagentStatus / SubagentEvent / ParentAgentStreamEvent 定义 |
| `src-tauri/src/subagent/spawn.rs` | 535 | `spawn_subagent()` 校验+派生入口、`execute_subagent()` 含 failover 重试和 plan mode 继承 |
| `src-tauri/src/subagent/injection.rs` | 466 | `inject_and_run_parent()` 等待空闲+恢复历史+流式注入、`PendingInjection` 队列、`flush_pending_injections()` 串行重试、`build_subagent_push_message()` 格式化 |
| `src-tauri/src/subagent/cancel.rs` | 69 | `SubagentCancelRegistry`：register / cancel / cancel_all_for_session / remove |
| `src-tauri/src/subagent/mailbox.rs` | 100 | `SubagentMailbox`（register / push / drain / remove）、`ChatSessionGuard`（RAII：ACTIVE_CHAT_SESSIONS + INJECTION_CANCELS + flush） |
| `src-tauri/src/subagent/helpers.rs` | 81 | `emit_subagent_event` / `emit_parent_stream_event` / `truncate_str` / `CleanupGuard`（RAII：移除 INJECTING_SESSIONS + flush）/ `cleanup_orphan_runs` / `mark_run_fetched` |
| `src-tauri/src/tools/subagent.rs` | 605 | 工具接口层：10 种 action（spawn / spawn_and_wait / check / result / list / steer / kill / kill_all / batch_spawn / wait_all）、`do_spawn` 共享逻辑、权限校验 |

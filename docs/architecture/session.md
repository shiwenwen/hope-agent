...
| 文件 | 行数 | 职责 |
|---|---|---|
| `src-tauri/src/session/mod.rs` | 9 | 模块声明和 re-export |
| `src-tauri/src/session/types.rs` | 284 | SessionMeta, SessionMessage, MessageRole, NewMessage 类型定义 |
| `src-tauri/src/session/db.rs` | 894 | SessionDB 核心实现（open, CRUD, FTS, 已读, Migration） |
| `src-tauri/src/session/helpers.rs` | 33 | auto_title + db_path 辅助函数 |
| `src-tauri/src/session/subagent_db.rs` | 200 | Subagent 运行记录 CRUD |
| `src-tauri/src/session/acp_db.rs` | 237 | ACP 运行记录 CRUD |

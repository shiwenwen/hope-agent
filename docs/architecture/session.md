...
基于 `last_read_message_id` 水位线机制：未读数 = `messages.id > last_read_message_id AND role = 'assistant'` 的记录数（SQL 子查询计算）。只计最终 `assistant` 行——一轮 turn 中的 `tool` / `text_block` / `thinking_block` / `event` 行都是同一个回复的组件或系统事件，不应重复计入徽标（否则单个问题触发 10 次工具调用就会显示 20+ 未读）。

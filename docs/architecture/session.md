...
| 文件 | 职责 |
|---|---|
| `crates/oc-core/src/session/mod.rs` | 模块声明和 re-export |
| `crates/oc-core/src/session/types.rs` | SessionMeta, SessionMessage, MessageRole, NewMessage 类型定义 |
| `crates/oc-core/src/session/db.rs` | SessionDB 核心实现（open, CRUD, FTS, 已读, Migration） |
| `crates/oc-core/src/session/helpers.rs` | auto_title + db_path 辅助函数 |
| `crates/oc-core/src/session/subagent_db.rs` | Subagent 运行记录 CRUD |
| `crates/oc-core/src/session/acp_db.rs` | ACP 运行记录 CRUD |
| `src-tauri/src/commands/session.rs` | Tauri 命令层（11 个命令） |
| `crates/oc-server/src/routes/sessions.rs` | — | HTTP 路由层（REST API） |

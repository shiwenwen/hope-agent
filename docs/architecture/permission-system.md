...
`mac_control` 的只读动作（`status` / `permissions` / `snapshot` / `wait` / `apps.list` / `apps.frontmost` / `apps.installed` / `apps.search` / `windows.list` / `menu.list` / `dialog.inspect`）直接放行；普通突变和隐私敏感动作（例如 `clipboard.get/set/clear`）弹审批；高风险突变动作禁用 AllowAlways。

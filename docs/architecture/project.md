...
ChannelWorker (channel/db.rs::ensure_conversation/resolve_or_create_session)
   ├── ⓪ 命中 channel_conversations 已有映射 → 复用旧 session（不重判项目，避免活跃会话被半路换 project_id）
   ├── ① find_by_bound_channel(channel_id, account_id) WHERE archived = 0
...
> **归档 = 失效绑定**：步骤 ① 的查询带 `archived = 0` 过滤——把项目归档后，新进的 IM 消息走"未命中"分支创建无项目会话，避免归档项目继续吞流量。要彻底解绑还是用 `bound_channel = null` 显式 patch。

...
| **ha-mcp** | McpManager 注册表 / client / 四种 transport / OAuth（PKCE，凭据 0600）/ watchdog / `mcp_resource`·`mcp_prompt` 工具 / owner API | `mcp` facade：wire 类型再导出、`mcp__` 前缀单一来源、auto-approve 信任谓词（安全语义留 kernel） | `mcp_hooks` 八件套（init/watchdog 起动 + 工具快照 / lazy catalog 自举 / 配置查表 / call_tool / prompt 段 / 设置 reconcile） |

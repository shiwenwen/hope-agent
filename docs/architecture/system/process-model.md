...
桌面壳 `window_visibility` 统一主窗口的关闭、Cmd+Q 隐藏及 Dock、托盘、单实例、菜单、快捷键重开。macOS 原生全屏拥有独立桌面空间，直接 `hide()` 不保证释放该空间；`set_fullscreen(false)` 返回也不表示退出动画已经完成。

主窗口在启动时按原生窗口对象监听 AppKit 的进入、退出全屏开始与完成通知。全屏或过渡中的关闭请求只登记一次待隐藏票据；进入完成后才请求退出，收到退出完成通知后才隐藏。原生动作离开通知回调后再在主线程执行，避免通知栈重入。重开会取消待隐藏票据，防止迟到的完成事件再次隐藏窗口；10 秒超时仅取消对应票据并记录警告，绝不强制隐藏仍在全屏过渡中的窗口。窗口几何持久化仍由 `window_state` 管理。

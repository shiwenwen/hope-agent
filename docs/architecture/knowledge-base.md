...
当前 Knowledge Compiler 的文字 / 文件 / 网页 / 音视频 / 图片资料闭环已经完成：资料舱、编译审阅、Evidence、查询归档、维护、外部 agent、PDF / DOCX / 多文件、浏览器采集、URL / Browser source refresh + 版本化、Source 变更驱动编译建议、STT 转录、图片 OCR、远程媒体 URL 归档、IM / 聊天附件归档、原始媒体可选留存与缩略图、外部 vault raw/source 文本快照同步、批量导入与去重治理都落在同一条 raw source → Review Diff → `.md` note 链路上。后续计划只记录方向，不代表当前版本必须实现。

1. **更强 Evidence 治理**：把 source 引用从 frontmatter / Markdown 约定升级为可查询的 evidence index，支持按 source 反查所有 compiled claims、stale source 批量提醒和引用覆盖率评分。
2. **导入流水线后台化**：当前 batch import 是 owner 命令同步执行、run/item 持久可观测；如果引入超大媒体、长转录或远程批量抓取，应迁入 `async_jobs::JobManager`，继续复用现有 run/item 表作为用户可见事实层。
3. **更细粒度相似治理**：当前相似 source 分组是确定性 shingle/Jaccard 视图；后续可加入 embedding 相似度、人工 merge / dismiss 记忆，以及跨 KB 的重复资料提示。

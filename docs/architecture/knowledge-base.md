...
| `source.rs` | Raw Source / 资料舱（D15 + 导入流水线）：文本 / Markdown / PDF / DOCX / 音频转录 / 视频转录 / 图片 OCR / URL / 浏览器快照导入，STT failover 转录，vision analysis agent OCR，SSRF-gated URL fetch，HTML→Markdown 可读快照，受控浏览器 tab DOM/选区采集，`extracted_text_hash` exact dedup，批量 import run / item 状态、失败重试、相似 source 分组，Hope 管理目录写盘，source chunk 切分，list/read/delete owner 面 |
...
- **Raw Source 隔离**：资料舱 source 是 owner-plane 输入快照，不注入 system prompt、不进入 `note_search` / `knowledge_recall`、不写外部 vault；URL 导入必须过 `security::ssrf::check_url`，重定向 host 也走同步拦截，最终 URL 再复检。浏览器采集只读当前受控 tab 的 DOM / selection，不通过后端重新 fetch URL，避免登录态内容丢失或 SSRF 语义混淆；音频 / 视频 / 图片导入只保留转录或 OCR 后的 Markdown 文本快照，原始二进制不持久化。所有 source 写入前以 `extracted_text_hash` 做 exact dedup，重复 item 在 import run 中标记为 `duplicate`；普通 source import 不允许伪造 `browser_snapshot`。
...
当前 Knowledge Compiler 的文字 / 文件 / 网页 / 音视频 / 图片资料闭环已经完成：资料舱、编译审阅、Evidence、查询归档、维护、外部 agent、PDF / DOCX / 多文件、浏览器采集、STT 转录、图片 OCR、批量导入与去重治理都落在同一条 raw source → Review Diff → `.md` note 链路上。后续计划只记录方向，不代表当前版本必须实现。

1. **Source 刷新与版本化**：对 URL / 浏览器快照支持手动 refresh，生成新的 source version 或新 source，并用 `extracted_text_hash` / diff 展示变化；避免静默覆盖旧证据。

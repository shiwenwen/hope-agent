...
**`KnowledgeSource`（真相源，`sessions.db`，D15）**：`id` / `kb_id` / `kind(markdown|text|pdf|docx|url_snapshot)` / `title` / `origin_uri` / `stored_path` / `content_hash` / `extracted_text_hash` / `status` / `compiled_at` / `created_at` / `updated_at` / `size`。Raw 文件不落外部 vault，统一存在 `~/.hope-agent/knowledge/{kb_id}/sources/{uuid}.md|txt`。PDF / DOCX 导入时 owner API 只接收用户选择的文件字节（`dataBase64`），后端落临时文件提取文本，再保存带元数据头的 Markdown 文本快照；原始二进制不持久化，文件名永远由后端 UUID 生成，不信任用户传入路径。

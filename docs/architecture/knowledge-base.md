...
| `db.rs` | index.db 后端：note/chunk/link/tag 写入（单事务重索引）+ FTS/vec 查询 + 反链 + 重解析 + `list_broken_links` / `list_orphan_notes`（维护面板）+ `all_resolved_links`（图谱边，WS1）+ **`block_backlinks`**（块级反链：`note_link.anchor = '^id' COLLATE NOCASE`，Batch G） |
| `parser.rs` | pulldown-cmark 扫 heading/code + **叶块 span**（paragraph/item/heading），正则扫 `[[ ]]` / `#tag` + **Obsidian `^block-id` 块锚**（跳过 code），D14 坐标（`PosMap` 码点 offset + 1-based line / 0-based col，相对原始全文）+ 手写 frontmatter→JSON。`ParsedBlock{block_id,start,end,text}`：行尾 `^id`（`[A-Za-z0-9-]+`，前导空白/行首）附到所在叶块（独占一行的 `^id` 附到上一块），`text` 剥锚——首个 id 胜出，**不落表**（transclusion 重解析、块反链查 `note_link.anchor`），`line_block_anchor` 供写工具复用 |
...
- **块级引用写入（Batch G，D14/三闸门）**：`note_assign_block({kb,path,block_text,block_id?,expected_file_hash?})` 给目标块加 Obsidian `^id`。`resolve_anchor_placement`：`block_text` 唯一命中（同 `note_patch` 0/多次拒）后**解析整个叶块**——段落多行时把 `^id` 落到**该块最后一行**末尾（不截块），列表项/标题按单行块；**幂等**检测覆盖整块（块尾行的尾随 `^id` 或块下方独占行的 `^id` 都算已存在，直接回该 ref，不写第二个）；**拒 frontmatter / 代码围栏命中**（无法承载真实块锚）。id 缺省由 `blake3(block_text)` 取 6/8/12/16 hex 防撞确定性生成（无 RNG），显式 id 校验 `[A-Za-z0-9-]+` + 防重（防重集 `collect_block_ids` 走原始行扫，覆盖未解析锚）。三闸门复用既有：`writable_scope`（外部只读/WS7）+ `guard_stale`（磁盘 raw BLAKE3）+ 唯一命中。返回的引用走 `stable_block_ref`：basename 经 resolver **唯一回指本笔记**才用 basename，否则用路径式 `[[folder/Note#^id]]`（防 `a/Note.md`+`b/Note.md` basename 撞车被 resolver 最短路径/字典序解析到别篇）。`note_backlinks` 加可选 `block` 参数 → `db::block_backlinks`（`^`/空白塌成空 id 直接拒）。
...
**笔记嵌入 transclusion（Phase 2 Batch B，WS2）**：预览/分屏的预览栏在有 `kbId` 时换 [`NoteTransclusionView`](../../src/components/knowledge/NoteTransclusionView.tsx)（否则退回纯 `MarkdownRenderer`）。纯函数 [`transclusionParse.ts`](../../src/components/knowledge/transclusionParse.ts)（`parseEmbedSegments` 跳代码围栏切出**整行** `![[ref]]` 块、`stripFrontmatter`，独立可测）把正文切成 markdown 段与 embed 段；embed **传完整 ref（含 anchor）** 经 `kb_note_read_ref_cmd`（owner resolver 单源）取目标——服务端按 anchor 切片（Batch G）；剥 frontmatter 后**递归**渲染，深度上限 4 + 循环检测 + broken/loading 占位。**循环检测 key = `relPath` + anchor**（`embedAnchor` 取；顶层用当前笔记 path 无 anchor 预置）：whole 自嵌 `![[A]]` 仍判环，但 anchored 自嵌 `![[A#^p1]]` / `![[A#Heading]]`（切片是不同块/段）正常渲染，真递归（同 anchored ref 套自身切片）仍判环（Batch G 修）。embed 结果按 `${kbId}::ref`（含 anchor）模块级缓存，`cacheBustKey`（KnowledgeView 的 `embedCacheKey`，随 `knowledge:changed` ++）整表失效。行内 `![[ ]]`（非独行）仍按原文渲染。
...
- **heading outline 弹层**：纯函数 [`outline.ts::parseHeadings`](../../src/components/knowledge/outline.ts)（ATX `#`×1–6、≤3 缩进、跳代码围栏、剥尾随 `#`，独立可测）→ [`HeadingOutline`](../../src/components/knowledge/HeadingOutline.tsx) 弹层（`useClickOutside` 范式），点小节 `setRevealTarget({line})` 精确跳转。仅 `mode ∉ {preview, outline}` 显示（outline 模式本身就是大纲）。
- **原生大纲只读视图（Batch G，D8 可选层）**：第 5 个视图模式 `outline`（`NoteEditorMode`）。[`outline.ts::buildOutline`](../../src/components/knowledge/outline.ts)（复用 `parseHeadings`，按 level 栈建标题树 + 每节自有正文 + 首标题前 preamble，纯派生不改 `.md`）→ [`OutlineView`](../../src/components/knowledge/OutlineView.tsx)（可折叠标题树，每节正文走 `MarkdownRenderer`，点标题 `onOutlineJump` → KnowledgeView 切回 `source` 再 `setRevealTarget({line})`——outline 无 CM6 view 可滚）。`NoteEditor` 在 `mode==="outline"` 时 early-return `OutlineView`（在全部 hooks 之后，与 preview-only 同样不挂 CM6 host）；AI 改写 / hover outline 在该模式隐藏（无 view 可 splice）。D8 红线：只读渲染、永不替代 CM6 底座 / 不破坏性转写。

...
- **Finding skills**: say “find a skill that can do this.” `ha-find-skills` checks existing capabilities first, then searches registries or GitHub and explains each candidate's purpose, source, license, and dependencies. A search-only request proceeds to installation only after you select and authorize a candidate.
- **Installing a selected skill**: say “install the skill at this GitHub link,” or invoke `/ha-skill-installer` with the link. The installer supports a single skill directory in a public GitHub repository, a chosen branch / tag / commit, or a local skill directory. Private repositories and other Git hosts use an already-authorized local checkout. Python 3.9+ is required. For GitHub sources, Hope’s bundled reader checks the complete file inventory and size limits before downloading content from the selected commit. Preparation shows the source, destination, and files for review, and installation uses that exact snapshot. An explicit authorization for the same installation does not require another confirmation.
- **Location and protection**: the default is `~/.hope-agent/skills/` (or the corresponding data directory when `HA_DATA_DIR` is set). Request “this project only” to use that project's `.hope-agent/skills/`. Existing and bundled skills are never overwritten. Downloaded scripts are not run and dependencies are not automatically installed. Installation remains subject to the current permissions and sandbox.
- **Post-install checks**: the AI refreshes discovery and verifies the installation path, enabled state, and missing dependencies. Successful installation does not mean dependencies are satisfied or the skill has run. Disabled states and conditional activation keep their existing meaning; review them in Settings → Skills.

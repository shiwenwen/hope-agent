...
2. **Model Provider (required)** — pick a service from about 50 built-in templates (Anthropic, OpenAI, DeepSeek, Tongyi Qianwen, and more) and enter an API key; with Codex you can sign in to your account directly via OAuth, no manual key needed. Templates change as releases add or retire options, so the list shown by Add Provider is authoritative. This step also embeds the "One-click local model install" card. The skip button is highlighted in red as a reminder.

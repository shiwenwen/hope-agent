...
| [知识库系统](architecture/knowledge-base.md)         | **设计草案**：独立 PKM 知识库（真实 `.md` + SQLite 旁路索引）、Wikilink 双链/反链、AI 第一公民级完整读写/检索/自主维护（三层能力）、Obsidian/Logseq 兼容矩阵、`WorkspaceScope::for_knowledge`、决策账本 + 分阶段路线图 | `knowledge/`（规划中）                            |

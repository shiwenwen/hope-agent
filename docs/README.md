...
| [Coding Eval 控制面评测](architecture/coding-eval.md) | Phase 3.7-4.2 确定性 fixture harness：临时 git repo + 真实 session/goal/task/workflow/IDE context/improvement state，回归 Review / Smart Verification / Context Retrieval / Improvement Loop 协同质量 | `coding_eval.rs`, `tests/fixtures/coding_eval/` |
| [Coding Improvement Loop](architecture/coding-improvement-loop.md) | Phase 4.2 质量趋势与学习闭环：基于 durable Goal/Workflow/Review/Verification/Eval 数据生成 trend report、workflow retro、failure taxonomy、proposal 队列，并可预览/应用/晋升 eval、workflow、guidance、skill 产物 | `coding_improvement.rs`, `coding_eval.rs`, `components/chat/workspace/` |

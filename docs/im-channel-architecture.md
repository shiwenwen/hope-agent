...
| 文件路径 | 说明 |
|---------|------|
| `src-tauri/src/chat_engine.rs` | 共享聊天执行引擎：EventSink trait + ChatEngineParams + run_chat_engine()，UI chat 和 IM channel 共用 |
...
| `src-tauri/src/lib.rs` | +mod channel, +mod chat_engine, +statics, +setup, +commands |
| `src-tauri/src/provider.rs` | +channels 字段 |
| `src-tauri/src/commands/chat.rs` | 核心 model chain 循环提取到 chat_engine，调用 run_chat_engine() |

...
| **Discord** | WebSocket Gateway | Bot Token | DM, Group, Forum, Channel | Application Commands 同步、RESUME 重连 |
| **Slack** | Socket Mode WebSocket | Bot Token + App Token | DM, Group, Channel | mrkdwn 格式、一次性 URL 重连 |
| **飞书 / Lark** | WebSocket 事件订阅 | App ID + App Secret | DM, Group | OAuth Token 自动刷新、多域名支持 (feishu/lark/私有部署) |
...
| 渠道 | 预期连接方式 | 优先级 | 说明 |
|------|------------|--------|------|
| **WhatsApp** | Baileys 逆向协议 / OpenClaw 桥接 | 高 | 全球最大 IM，缺官方 API，维护风险高 |
| **Signal** | signal-cli REST 桥接 | 中 | 隐私优先用户群，需外部 signal-cli 进程 |
| **IRC** | 原生协议 | 中 | 协议极简，Rust `irc` crate 成熟 |
| **Google Chat** | Webhook / Chat API | 中 | 企业场景，Webhook 模式实现简单 |
| **iMessage** | BlueBubbles REST API | 低 | macOS 限定，需 BlueBubbles 服务端 |
| **LINE** | Webhook Bot API | 低 | 日本/东南亚市场 |

以上规划中渠道已在 `ChannelId` 枚举中预留变体，实现时只需添加 plugin 模块。
...
兼容 OpenClaw 的 `CHAT_CHANNEL_ORDER`，扩展了 Feishu 和 QqBot：
...
|------|---------|-------------|--------|
| **ChannelId** | 9 built-in + extensions | 11 built-in + Custom(String)（扩展了 Feishu/QqBot） | 完全兼容 |
...
| **WebSocket** | 各渠道独立 SDK | 统一 `ws.rs` + reqwest | 共享传输层 |
| **Plugin 模式** | 8 个子适配器 trait | 1 个扁平 trait | Rust 惯用简化 |
| **传输层** | polling + webhook + WS | polling + WebSocket | 部分实现（缺 webhook） |
| **流式输出** | Draft Stream Loop（编辑消息） | Telegram Draft + send/edit 模式 | 已实现（Telegram） |
| **已实现渠道** | 24 个 | 6 个 (Telegram/WeChat/Discord/Slack/Feishu/QQ) | 覆盖核心场景 |
...
| `src-tauri/src/channel/types.rs` | 核心数据类型（ChannelId 11+Custom 变体） |
...
| 文件路径 | 说明 |
|---------|------|
| `src/components/settings/ChannelPanel.tsx` | 渠道设置面板（6 渠道凭据表单） |
| `src/components/common/ChannelIcon.tsx` | 渠道图标（Telegram/Discord/WhatsApp/WeChat/Slack/Feishu/QQ） |

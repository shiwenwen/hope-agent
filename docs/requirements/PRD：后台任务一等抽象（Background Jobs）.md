...
- ⏳ **待续**：R4（前端后台任务面板 + 徽标 + 完成通知 + 注入合并窗口；消费 R3 的 `job:*` 事件 + `list_jobs`/`cancel_job` 端点 + 12 语 i18n 文案）、R7 per-kind 配额、R8（AwaitingApproval 落地）、R9（设置三件套横切）。**P0 地基 + P1 后端事件 + P2 编排已闭环；R4 是 P1 的前端收尾（需用户文案 / 设计输入）。**

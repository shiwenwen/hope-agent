...
- ✅ **R7.1 分层配额闭环 — per-session 强约束 + auto-bg 计入**：把「单一全局数」升级为**全局 + 每会话**两层硬配额,并修补 auto-background 不占槽的漏。
  - **每会话硬上限**:`async_tools.maxConcurrentJobsPerSession`（默认 6,`0` = 不限）。此前 per-session 计数**仅用于排队公平排序**,单会话仍能占满全局池饿死其他会话;现 `SlotManager::reserve_inner` 同时校验全局 + 每会话两层,任一满即入队。调度器 `pick_fair_index` 跳过已达每会话上限的会话（防 head-of-line:capped 会话的队头不阻塞其他会话的提升）,空位再起靠 RAII `SlotReservation` Drop 唤醒。
  ...
  - **三件套**:`maxConcurrentJobsPerSession`（MEDIUM,AsyncToolsPanel + 既有 `async_tools` category 泛型 merge read/update + SKILL.md 登记)。i18n `settings.asyncToolsMaxConcurrentPerSession{,Desc}` 全 12 语。slots 单测覆盖每会话拒绝 / 跳过 capped 会话 / forced 越 cap;config 单测覆盖默认 6 + `0` 不限。
- ⏳ **待续**：R7.4（重试 per-kind + 共享 executor,§8.2 标「需先勘探」）、subagent reject→queue 真统一（高风险大改）、R8（AwaitingApproval 落地,需审批引擎,#318 已合 main）、R9（设置三件套横切剩余:wakeup clamp / 队列上限 / output_tail 尺寸 / 进度节流)。**P0 地基 + P1（看得见,后端事件 + 前端面板）+ P2 编排 + R7.1 分层配额闭环已全部完成。**

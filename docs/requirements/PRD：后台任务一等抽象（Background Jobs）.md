...
- ⏳ **待续**：R7.4 共享 executor（Send 工具用 tokio 任务 + 信号量替代 thread-per-job,§8.2 标「需先勘探不可一刀切」——独立一刀)、subagent reject→queue 真统一（高风险大改、不在验收清单）、**后台 subagent 内层审批的 AwaitingApproval 投影**(R8 follow-up,需 approval 事件监听映射 session→投影 job)、R9（设置三件套横切剩余:wakeup clamp / 队列上限 / output_tail 尺寸 / 进度节流)。**P0 地基 + P1（看得见,后端事件 + 前端面板）+ P2 编排 + R7 调度健壮性（分层配额 + 重试 + 结构类拒绝验收）+ R8（AwaitingApproval 落地）已全部完成；R7 剩共享 executor 这一可选重构。**

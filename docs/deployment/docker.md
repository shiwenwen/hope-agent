...
| `HA_KNOWLEDGE_AGENT_READ_TOKEN` | _未设置_ | Phase 7 Knowledge Agent 只读 token。只能访问 `/api/knowledge/agent/{search,read,expand,sources}`，不能访问 owner 管理 API 或 `compile/propose`；适合给外部 agent 的 HTTP 脚本使用 |

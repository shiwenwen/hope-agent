...
### F-106 IM `try_handle_approval_reply` 真路径 e2e 测试缺失

- **来源**：2026-05-17 #218 post-merge `/simplify` review。当期 followup PR 把两个误导性 mock 测试改名为 `text_pending_list_pop_is_lifo_for_bare_verb` / `text_pending_id_tag_position_match_routes_to_non_top` + 注释承认是 list-helper 测，并加 `drop_pending_by_request_id_clears_across_chats` 覆盖新 helper。但 LIFO 与 `#tag` 后缀 routing 仍没有端到端 `try_handle_approval_reply → submit_approval_response` 路径测试。
- **现象**：[`channel/worker/approval.rs` tests](../../crates/ha-core/src/channel/worker/approval.rs) 验证了 list 操作 + parser + helper，但 `try_handle_approval_reply` 本身没被测试调过。dispatcher 调用契约的 routing 决策（pop vs position+remove）仍是「mirror in test code」。
- **为什么留**：真路径测试需要 `cfg(test) pub(crate)` register helper 反向钻入 `tools::approval::PENDING_APPROVALS`（私有 struct），且要构造完整 `MsgContext`。改动面溢出 followup 范围。
- **改的话要做什么**：
  1. [`tools/approval.rs`](../../crates/ha-core/src/tools/approval.rs) 加 `#[cfg(test)] pub(crate) async fn register_pending_for_test(request_id, session_id) -> oneshot::Receiver<ApprovalResponse>`
  2. [`channel/worker/approval.rs`](../../crates/ha-core/src/channel/worker/approval.rs) 测试改成 register PENDING + register TEXT_PENDING + 构造 `MsgContext` + 调 `try_handle_approval_reply` + assert receiver 收到正确 response
- **影响面**：覆盖空白 —— refactor dispatcher 路径不会被现存测试 catch。当期靠 parser/helper 单元测 + 手动 IM 验证兜底。
- **触发时机建议**：下次动 dispatcher 或加 routing case（如优先级队列 / 显式 `oldest`/`newest` 关键字）时顺手补。

---

### F-107 IM 审批 `#tag` 后缀拼错时的 UX 模糊

- **来源**：2026-05-17 #218 post-merge `/simplify` review。
- **现象**：用户输入 `yes#abc`（明显在尝试回复审批）但 tag 拼错时，[`try_handle_approval_reply`](../../crates/ha-core/src/channel/worker/approval.rs) 返 `false` → dispatcher 把消息当新 turn 喂进 LLM + `maybe_send_pending_hint` 节流提示「Treating this as a new message」。语义模糊：用户既看到「当作新对话」又看到「你有 N 个 pending」，不知道审批到底有没有响应。
- **为什么留**：当期 followup 已经修了 IM `TEXT_PENDING` 泄漏 + parsing 扩展 + #tag routing + 超时通知 + 节流，是文本审批可用性的 P0 集合；这条是 P2 UX 打磨，单独迭代收益更清晰。
- **改的话要做什么**：把 `#<tag>` 失配从 fall-through 改为「消费消息 + 显式提示『#<tag> 没匹配上，pending tag 是 #aaaaa / #bbbbb，请回复 yes#aaaaa 或 no#bbbbb』」。需要 [`try_handle_approval_reply`](../../crates/ha-core/src/channel/worker/approval.rs) 新增「带后缀但找不到」分支返 true + 发提示。
- **影响面**：UX 微瑕，无功能 bug。带 `#tag` 后缀的用户已是 power-user 行为，拼错本身不常见。
- **触发时机建议**：用户反馈或 IM 真实回放出现这种 case 时再修。

---

### F-108 IM 审批 alias 表抽 const arrays + `HINT_THROTTLE` 配置化

- **来源**：2026-05-17 #218 post-merge `/simplify` review（两条合并）。
- **现象**：
  1. [`parse_approval_reply`](../../crates/ha-core/src/channel/worker/approval.rs) 的 `match lower.as_str() { "yes" | "y" | "ok" | ... }` 三行 pattern 长度 18+ 个字符串字面量，可读性差且 lint/test 无法单独扫
  2. `HINT_THROTTLE = 60s` 是 magic number，没暴露到 config —— 用户嫌烦想调节频率没入口
- **为什么留**：低优 UX 打磨，本次 followup 聚焦泄漏 + 流程问题。
- **改的话要做什么**：
  1. 抽 `const ALLOW_ONCE_ALIASES: &[&str] = &[...]` / `ALLOW_ALWAYS_ALIASES` / `DENY_ALIASES` 三组 const；`parse_approval_reply` 改为 `if ALLOW_ALWAYS_ALIASES.contains(&lower.as_str()) { ... }` 链式；加 doc test 拍下白名单。便于未来加日 / 韩 / 西等语言
  2. `permission.imApprovalHintThrottleSecs` 加到 `AppConfig.permission`，默认 60s
- **影响面**：可读性 + 用户控制度；无功能问题。
- **触发时机建议**：用户社区贡献 alias 翻译时一起搞；或者收到「调节提示频率」reqeust 时一起做。

---

### F-109 `hope-agent server` 启动 banner 在 `--version` / `--help` 之前

- **来源**：2026-05-17 #219 post-merge `/simplify` review。
- **现象**：[`hope-agent.rs`](../../crates/ha-server/src/bin/hope-agent.rs) 在 `--version` (line 52) / `--help` (line 80) 短路之前就解析 `--dangerously-skip-all-approvals` + `HA_SERVER_AUTO_APPROVE_TOOLS` 并打 stderr banner。`hope-agent --version` 时如果 env 设了 `HA_SERVER_AUTO_APPROVE_TOOLS=1`，banner 会先打到 stderr 再打版本到 stdout。CI 里 `hope-agent --version 2>/dev/null` 还好；裸跑会看到无关警告。
- **为什么留**：`--dangerously-skip-all-approvals` 早就有同样问题；既然新加的 env-driven flag 触发面更大，单独处理更对称。本期 followup 聚焦审批语义 bug。
- **改的话要做什么**：把 flag/env 解析挪到 `--version` / `--help` 短路之后；或仅在 `args[1] == "server"` 路径上检测 + 在 [`run_server`](../../crates/ha-server/src/bin/hope-agent.rs) 之前再 set。
- **影响面**：UX 微瑕；不影响功能。
- **触发时机建议**：下次动 `hope-agent.rs` argv 解析或加新 server flag 时顺手做。

---

### F-110 i18n 12 文件批量插入会触发不相关字典重排噪声

- **来源**：2026-05-17 #220 post-merge `/simplify` review（efficiency agent）。
- **现象**：当期用 Python 脚本 `dict(sorted(settings.items()))` 给 12 个 locale 加新 key 时，`json.dump` 会把整段 `settings` 字典按字母序重写，导致**两处与本 PR 完全无关的 key 顺序变动**（`permUnknown` 移位 + `webSearchProviderBrave` / `webSearchProviderBocha` 互换）。12 个 locale 全部出现同款噪声，污染 git blame —— 以后 grep `permUnknown` 改动历史会撞到当时的 PR commit。
- **为什么留**：流程问题（脚本写法），不是代码 bug。已合并不回炒；下次注意。
- **改的话要做什么**：选其一：
  1. 脚本只重排「新增 key 所在父对象的局部」（如 `agentAsyncToolPolicy` 那一段），不动其他段
  2. 跑一次专门的 chore commit `chore(i18n): sort all locale dicts alphabetically` 一次性付清存量，之后所有 feat PR 都只动新增 key 的临近行
  3. 用 `node scripts/sync-i18n.mjs --apply` 走仓库提供的工具而不是 ad-hoc Python（如果该工具能保序）
- **影响面**：review 流程；git blame 干净度。无功能影响。
- **触发时机建议**：下次需要批量加 i18n key 时换用 #1 或先做 #2。

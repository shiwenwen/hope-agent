...
7. 接剩余 async host bridge：`askUser`，把 `spawnAgent/waitAll` 补真实 fan-out E2E，把 `validate` 升级为 async job attach，并补通用 Started async attach / 端到端重启恢复验证。

...
- host API MVP（已完成同步首批、`tool/read/grep` dispatch bridge、`workflow.map` fan-out 物化、`spawnAgent/waitAll` subagent bridge、targeted `validate`、`askUser`、`diff`）。
...
7. 接剩余 async host bridge：把 `spawnAgent/waitAll` 补真实 LLM fan-out E2E，把 `validate` 升级为 async job attach，并补通用 Started async attach / 端到端重启恢复验证。

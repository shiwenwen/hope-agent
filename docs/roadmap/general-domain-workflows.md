...
- Summary 覆盖 workflow drain 时间、失败/阻塞/取消/活跃、owner control intervention、审批请求/决策/已闭环等待耗时、未闭环 open wait、暂停/恢复/取消/恢复事件、workflow output-token budget usage/exhaustion、loop tick、campaign item retry / failed / interrupted、connector E2E execution / verification evidence、缺连接器执行后复核 warning、最近活动时间 / 距今时长，以及 `sampleDays / requiredSampleDays` 跨天样本覆盖。
...
- Dashboard Learning 新增「Domain soak report」卡片，展示 workflow / loop / campaign / connector 样本量、connector verification 数、跨天覆盖、样本新鲜度、critical/warning incidents、最大 drain 时间、最近 timeline 和下一步建议。
- 新增核心单测覆盖：1 天窗口内 drained workflow + loop + campaign + connector execution+verification evidence 时 passed；连接器动作已执行但缺执行后复核时只能 `insufficient_data`；多天窗口单日样本只能 `insufficient_data`；跨天且新鲜样本可 passed；failed workflow + active campaign item 时 failed，并输出 critical/warning incidents 和 Markdown。

验收：

- 没有样本的窗口不能 passed，只能 `insufficient_data`。
- 有 critical incident 或内嵌 Operational Gate failed 时必须 failed。
- active / queued / awaiting approval 不伪装成失败，但会让报告保持 `insufficient_data`，提醒用户先 drain；connector execution evidence 缺 `connector_action_verified` 时同样保持 `insufficient_data`，避免外部动作只记录执行、不读回复核就被当成 E2E 稳定样本。
- 多天窗口默认要求至少 2 个不同自然日的真实活动样本；单日样本或最近活动超过 24 小时时不能 passed，会保持 `insufficient_data` 并在 freshness / sample-days 指标和 recommended next step 中提示补新样本。1 天窗口只要求 1 天样本，避免短窗口验收被不可能的跨天要求卡住。
...
- 工作台新增「真实样本验收」卡片：从当前 session 的 domain evidence、Operational Gate、Soak Report、Artifact Export Guard 与 Connector Action Guard 派生覆盖领域数、控制面记录数、已排空样本、样本新鲜度、输出预算健康、Connector E2E evidence、验收进度与事故/缺口；验收进度按必需项通过数 / 必需项总数保守计算，failed gate 会拉低必需项进度，守门通过要求至少有已观察 gate 且全部 passed，并把跨领域覆盖作为额外加分，让用户直接看到“还差多少”；卡片可见区新增与复制报告同源的 `acc-xxxxxxxx` 验收快照 ID，方便用户和 reviewer 对齐当前卡片、采样清单与 Markdown 验收报告是否来自同一批可见状态；卡片、复制报告和「采样清单」任务使用同一个 review context 生成快照 ID，避免同一轮验收材料出现两个 ID；卡片新增只读「验收结论」和「证据等级」：验收结论把 danger gap、未通过必需项、warning/info gap 和控制面样本数归并成 `不可验收` / `待补样本` / `可局部复核` / `可验收`，证据等级把同一份材料标成 `未采样` / `阻塞样本` / `局部样本` / `局部验收` / `非外部动作候选` / `真实 E2E 候选`，明确它只能做采样待办、问题定位、局部复核、非外部动作验收，还是已经包含连接器执行/复核 evidence 可进入最终人工复核；来源分布会按 evidence 可见 metadata 统计 workflow、connector、fixture/mock、manual、public 和 restricted 计数，控制面组成会按 Soak / Operational summary 展示 workflow、loop、campaign 与 connector E2E 的完成/总量，帮助 reviewer 看出当前验收包到底由真实 agent、连接器、fixture/mock、人工记录还是长任务控制面历史构成；这些字段进入 UI、复制报告和采样清单，方便人工 / Claude Code review 一眼判断这份样本能否作为最终验收证据，且都不是新后端 gate；必需项清单会逐项显示领域样本、证据链、排空样本、样本新鲜、跨天覆盖、预算健康、事故清零、守门通过、连接器 E2E（仅涉及外部动作时）的通过 / 待补 / 阻塞原因；卡片新增「验收矩阵」，把 Workflow、Loop、Campaign、连接器 E2E、跨领域覆盖拆成独立样本跑道，显示当前 evidence 数字和下一步采样动作，未完成跑道可显式「转任务」；矩阵进入采样清单和复制报告，并带上每条跑道的证据 checklist / 刷新目标，但不改变 readiness 百分比，也不把未涉及外部动作的普通会话强行判 failed；样本新鲜与跨天覆盖来自 Soak Report 后端 summary：多天窗口至少要求 2 个不同自然日的真实活动样本，单日样本或最近活动超过 24 小时时只能保持 `insufficient_data`，Workspace 会把它显示为待补并可转任务；预算健康来自 Soak Report 的 output-token budget usage/exhaustion，预算耗尽会阻塞验收并可转任务，但不会自动改预算或重跑 workflow；验收缺口按 `阻塞` / `待补` / `扩展` 标记并排序，让 critical soak incident、连接器执行后复核缺失、跨天样本不足、样本陈旧、预算耗尽、failed E2E gate 等先于普通补样本展示；卡片可复制 Markdown 验收报告，供人工 / Claude Code / PR review 复核且不写 evidence，报告包含当前指标、必需项、验收结论、证据等级、来源分布、控制面组成、审计索引、复核协议、验收矩阵、跑道证据标准、缺口、守门状态、最近 evidence provenance、长跑事故 / timeline 和推荐下一步，但不展开完整 `sourceMetadata` 或私密正文；审计索引提供 `acc-xxxxxxxx` 快照 ID，并汇总领域、控制面样本、样本新鲜度、跨天覆盖、连接器执行/复核数、预算、Gate 快照、Evidence IDs 和 Soak 窗口；该快照 ID 是非安全确定性指纹，只用于对齐两份验收材料是否来自同一批可见摘要 / gate / evidence 状态，不替代真实 evidence 或完整性签名；复核协议明确只有 `可验收` 才能作为最终验收证据、转任务/按钮点击不能替代真实 evidence、长任务必须复核 Gate / Soak、连接器 E2E 必须包含测试账号或沙箱数据、执行、复核和回滚说明；每个未通过必需项、未完成样本跑道与验收缺口都可显式「转任务」；只要存在验收缺口、未通过必需项或非 optional 的样本跑道未完成，即使只有一个扩展缺口，也可一键创建包含当前指标、验收结论、证据等级、来源分布、控制面组成、审计索引、复核协议、验收进度、最近样本年龄、跨天覆盖、连接器执行/复核数、输出预算状态、必需项逐项状态、验收矩阵、带严重度标签的验收缺口和采样动作的「采样清单」任务，避免“真实样本还没跑够”只停留在文档提醒。
...
- 面板会根据证据缺口、P0/P1 review finding、验证失败、领域复核阻塞、交付守门、外部动作守门和 Soak incident 状态生成“下一步”提示；运行稳定性 check 和 recommended next steps 可「转任务」，并显示最长未排空 active work 时长；长跑审计会展示样本新鲜度、跨天覆盖、连接器执行后复核数量、最近 timeline、已闭环/未闭环审批等待、owner control intervention、output-token budget 消耗/耗尽信号，并可复制 Soak Markdown 报告；每条提示、每个守门 check、每条需复核 evidence、每条 Soak Report recommended next step 和每个长跑事故都可由用户显式点击「转任务」落入 TaskProgressPanel 追踪。

...
- Dashboard Learning 新增「Domain soak report」卡片，展示 workflow / loop / campaign / connector 样本量、跨天覆盖、样本新鲜度、critical/warning incidents、最大 drain 时间、最近 timeline 和下一步建议。
- 新增核心单测覆盖：1 天窗口内 drained workflow + loop + campaign + connector evidence 时 passed；多天窗口单日样本只能 `insufficient_data`；跨天且新鲜样本可 passed；failed workflow + active campaign item 时 failed，并输出 critical/warning incidents 和 Markdown。

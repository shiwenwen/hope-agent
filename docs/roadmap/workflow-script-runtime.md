...
状态：2026-06-30 已完成第 1 项 durable store/state machine、第 3 项 Primary-only startup recovery runner，并完成第 4 项的 runtime foundation 子集：QuickJS/rquickjs 受控执行、Script Gate 执行前阻断、`Date.now` / `new Date()` / `Math.random` runtime throw、位置化 op-key、`task.create/update` / `fileSearch` / `tool/read/grep` / `spawnAgent/waitAll` / targeted `validate` / `askUser` / `diff` / `trace` / `finish`、Completed op replay 无重复副作用、Started non-idempotent op fail-closed Blocked。第 2 项 fan-out 物化已接入 subagent host bridge、`spawnAgent` child_handle attach 与 replay 单测；真实子代理 E2E、validate async job attach、通用 Started async attach 仍待实现。

...
> 返回 [Phase 2 Workflow Mode 与 Script-first Dynamic Workflow 方案](phase2-coding-mode-dynamic-workflow.md)
> · [Phase 2 产品级完成审计](phase2-product-audit.md)
>
> 更新时间：2026-07-03
>
> 状态：实现收口清单。2026-07-03 已按 Claude Code dynamic workflows 心智补齐 Workflow Mode：开启后模型可自主决定是否通过 `workflow_run` 写脚本并启动后台 run；workflow 能力是通用长任务执行面，coding 是第一组深度模板。

## 完整目标

Phase 2 不是只加一个 `/mode` 开关，而是要把长任务变成可恢复、可观察、可审批、可验证的闭环。它最先服务 coding，但不是 coding-only：

1. Hope-native coding skills 可作为核心策略，不依赖第三方移植 skills。
2. Workflow Mode 可在 session 级开启模型自主动态编排；开启后模型看到 `workflow_run` 并可按任务复杂度决定是否写脚本启动后台 run。
3. Script-first workflow runtime 可执行模型生成的动态脚本，并通过 Script Gate / permission preview / approval 审核。
4. workflow op 身份由 runtime 位置化 op-key 决定，模型不手写稳定 id。
5. workflow 可编排 `task`、`fileSearch/read/grep/tool`、`spawnAgent/waitAll`、`validate`、`diff`、`askUser`、`trace`、`finish`。
6. 长任务可恢复、可取消、可暂停/恢复、可查看 trace，且不重复已完成副作用。
7. guarded repair loop 至少具备运行时 stop guard：重复验证失败、连续无有效 diff 进展会停止并记录原因。
8. GUI 不只显示命令结果；输入框必须能开启/关闭 Workflow Mode 并常驻显示状态；Workspace 必须能设置 Workflow Mode 与 Execution Mode；标题栏必须有可发现的 Workspace/Workflow 入口，且在有 active / waiting / failed workflow 时显示状态 badge；没有 run 时必须有可操作空态，能看到当前 workflow mode / execution mode / 工作目录状态并直接展开创建入口；随后可从目标生成可预检 workflow 草稿，普通路径不强迫用户先读脚本，脚本编辑收进高级区；用户能看到 run 总览、进度、审批焦点、授权清单、trace、validation 命令明细、agents、失败原因和恢复建议；历史 run 超过首屏预览时必须可展开选择；长 run 的失败 / 运行中步骤和关键事件必须优先浮出；Trace 的 op/event 原始详情必须可展开和复制，不能只依赖 hover tooltip；并可执行 run draft / approve / pause / resume / cancel，且 cancel 前必须确认会停止 run 与 workflow-owned children。
9. 至少 6 个 coding eval/smoke 场景有明确证据口径，后续接 Phase 0 baseline 不回归；通用场景层在 Phase 7 继续扩展。
...
| Workflow Mode / `workflow_run` | `sessions.workflow_mode` + Tauri/HTTP `get_workflow_mode` / `set_workflow_mode`；输入框和 Workspace 双入口支持 `off/on/ultracode` 并同步状态；system prompt 仅在开启时注入 Workflow Mode policy；模型工具 schema 仅在开启且非 incognito 时追加 `workflow_run`；执行层再次校验 session / incognito / mode，创建 run 后默认启动 preflight / approval / runtime                                                                                                                                                                                                                                                                                                                                     |
...
- `cargo check -p ha-core --tests`
  - 覆盖 Rust workflow runtime / tests 编译边界，并覆盖 Workflow Mode prompt 注入测试。
...
- 输入框必须有 Workflow Mode 入口，支持 `off/on/ultracode`，开启后输入框上方常驻显示状态，且和 Workspace 控件双向同步。
- 标题栏必须有可见的 Workspace / Workflow 入口，点击后打开同一个 Workspace / Workflow 控制台；有 active / waiting / failed run 时入口必须显示 badge，用户不需要打开面板才知道长任务在等自己。
- 没有 run 时也能切换当前会话的 Execution Mode：`off` / `guarded` / `deep` / `autonomous`。
- 没有 run 时必须显示可操作空态：展示当前 execution mode 和工作目录状态，并提供主操作直接展开创建表单。
- 可从 Workspace 直接新建 workflow：普通路径先填写目标并生成可预检草稿，草稿默认编排观察、子 Agent 实现、等待结果、验证、diff/产物检查、finish；脚本编辑默认收在高级区，高级路径仍可填写 kind、选择 execution mode、编辑 `workflow.js`、选择是否创建后立即运行。

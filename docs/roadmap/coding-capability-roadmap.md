...
9. **Phase 7：通用场景层与 Domain Workflow**。P6 完成后已转入通用场景层，Phase 7.1-7.3 已完成 Domain Workflow Registry、General Evidence Model 与 Domain Context Retrieval 第一版；后续继续把 Review / Verification / Learning Loop / Eval Gate 复用到 Research、Writing、Data Analysis、Meeting Prep、Knowledge Curation、Inbox、Project Ops 等非编程长任务；详细路线见 [通用场景层与 Domain Workflow 路线图](general-domain-workflows.md)。
...
- 已有第一层 coding eval harness、Phase 5.1 task-level scorer、Phase 5.2 agent execution runner、Phase 5.3 Gold Task Pack v1、Phase 5.4 strategy effect evaluator、Phase 5.5 Gold Task Pack 全量自动化、Phase 5.6 mock tool-call 基线、Phase 5.7 策略效果趋势持久化 / Dashboard、Phase 5.8 release gate、Phase 5.9 外部模型基线 runner、Phase 5.10 Learning Generalization Gate、Phase 6.1 Benchmark Run Center、Phase 6.2 Benchmark Campaign Runner、Phase 6.3 Cross-model Leaderboard、Phase 6.4 Real Task Corpus Expansion、Phase 6.5 Benchmark Report Export 与 Phase 6.6 Continuous Benchmark Gate / Improvement Backlog；Phase 7.1-7.3 已完成通用场景层地基与 domain context。
...
这部分不在本文展开，详细规划见 [通用场景层与 Domain Workflow 路线图](general-domain-workflows.md)。Phase 7.1-7.3 已先落地 domain workflow registry、workflow draft preview、通用 evidence 持久化、Goal evidence 链接与 domain context retrieval。核心原则是：模型仍可自由决策和动态编排，但 domain workflow 要守住关键证据、验证、用户确认、隐私权限和完成审计，提升非编程长任务的稳定下限。

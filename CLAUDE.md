...
- **统一 Tool 架构**：所有 tool 定义和执行逻辑集中在 `tools.rs`，通过 `ToolProvider` 枚举 + `to_provider_schema()` 自动适配不同 LLM 的 schema 格式。内置 8 个工具：`exec`（Shell 命令，支持 cwd/timeout/env/background/yield_ms/pty/sandbox，默认超时 1800s，login shell PATH 解析，动态输出截断，Docker 沙箱隔离）、`process`（后台进程管理：list/poll/log/write/kill/clear/remove）、`read_file`、`write_file`、`patch_file`（搜索替换编辑）、`list_dir`、`web_search`（DuckDuckGo）、`web_fetch`（URL 内容抓取）

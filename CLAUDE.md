...
- **Plan Mode（计划模式）**：`plan.rs` 实现可视化交互的三态 Plan Mode（Off/Planning/Executing）。Planning 状态自动禁用文件修改工具（write/edit/apply_patch/canvas）+ 注入 Plan Mode 系统提示词，LLM 只能读文件/搜索/浏览网页。Executing 状态注入计划内容 + 开放所有工具 + 条件注入 `update_plan_step` 内部工具实时报告步骤进度。Per-session 状态管理（内存 HashMap + DB `plan_mode` 列）。Plan 文件持久化到 `~/.opencomputer/plans/{session_id}.md`，Markdown Checklist 格式自动解析（`### Phase N:` + `- [ ]` / `- [x]`）。`/plan` 斜杠命令（enter/exit/approve/show）。6 个 Tauri 命令。Tauri 全局事件 `plan_step_updated` / `plan_mode_changed` 驱动前端实时更新。前端：ChatInput 工具栏 Plan 按钮（蓝/绿/灰三态）+ 蓝色横幅提示 + 内嵌 PlanBlock 可折叠卡片 + 右侧 PlanPanel 详情面板（进度条 + 分阶段步骤列表 + 审批操作栏）。子 Agent 继承 Plan Mode 工具限制防止逃逸

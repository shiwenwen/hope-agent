...
<tr><td><b>🦙 One-click local models</b></td><td>The Settings → Model page detects your hardware (macOS unified memory, Windows / Linux dedicated VRAM with system RAM as fallback) and recommends a Qwen3.6 or Gemma 4 size that fits in half of the available budget — Qwen3.6 27B / 35B-A3B and Gemma 4 e2b / e4b / 26B-A4B / 31B are all in the catalog. One click installs Ollama (via the official script on macOS / Linux, or sends Windows users to the download page), pulls the model, registers an OpenAI-compatible provider at <code>127.0.0.1:11434</code>, and switches the active model — no terminal required.</td></tr>

...
export default function RecapTab({ initialReportId }: Props) {
  ...
  const [rangeDays, setRangeDays] = useState<"incremental" | "7" | "30" | "90">(
    "incremental",
  )
  ...
  const loadList = useCallback(async () => {
    try {
      const rows = await getTransport().call<RecapReportSummary[]>(
        "recap_list_reports",
        { limit: 50 },
      )
      setReports(rows)
    ...
    const mode: GenerateMode =
      rangeDays === "incremental"
        ...
        : {
            ...
            filters: {
              startDate: new Date(
                Date.now() - parseInt(rangeDays) * 86400_000,
              )
                .toISOString()
                .slice(0, 10),
  ...
  const onDelete = useCallback(async () => {
    if (!selected) return
    if (!confirm(t("recap.confirmDelete"))) return
    try {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <Kpi
          label="Cost"
          value={`$${cur.estimatedCostUsd.toFixed(2)}`}
        />
        <Kpi
          label="Avg TTFT"
          value={cur.avgTtftMs ? `${cur.avgTtftMs.toFixed(0)} ms` : "—"}
        />
      </div>
      ...
      {sections.map((s) => (
        <div
          ...
          className={cn(
            ...
            s.key === "at_a_glance" && "bg-gradient-to-br from-indigo-50 to-sky-50 dark:from-indigo-950/40 dark:to-sky-950/30",
          )}
        >
      ...
      {facetSummary.totalFacets > 0 && (
        <div className="rounded-lg border bg-card p-4">
          ...
          <FacetBars
            title="Outcomes"
            items={facetSummary.outcomeDistribution}
          />
          <FacetBars
            title="Friction sources"
            items={facetSummary.frictionTop}
          />
        </div>
      )}
    </div>

...
function TeamMessageBubbleImpl({ message, members }: TeamMessageBubbleProps) {
  const sender = members.find((m) => m.memberId === message.fromMemberId)
  ...
  return (
    <div className="flex flex-col gap-0.5 px-3 py-1">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className={cn("text-xs font-medium")} style={{ color: senderColor }}>
          {senderName}
        </span>
        {message.toMemberId && (
        ...
        <span className="ml-auto text-[10px] tabular-nums text-muted-foreground/60">{timeStr}</span>
      </div>

      {/* Content */}
      <div className="pl-4 text-sm text-foreground leading-relaxed">{message.content}</div>
    </div>
  )
}

export const TeamMessageBubble = TeamMessageBubbleImpl

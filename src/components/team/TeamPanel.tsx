...
export function TeamPanel({
  ...
  const handlePause = useCallback(async () => {
    await getTransport().call("pause_team", { teamId }).catch(() => {})
  }, [teamId])

  const handleResume = useCallback(async () => {
    await getTransport().call("resume_team", { teamId }).catch(() => {})
  }, [teamId])

  if (!team) {
    return (
      <div
        className="flex items-center justify-center border-l border-border bg-background text-sm text-muted-foreground"
        style={{ width }}
      >

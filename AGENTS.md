...
**所有配置写入必须走 `config::save_config()` 或 `user_config::save_user_config_to_disk()`**，禁止 bypass 直接 `std::fs::write(config_path, ...)`。两个函数会在写入前通过 `backup::snapshot_before_write()` 自动把旧文件快照到 `~/.hope-agent/backups/autosave/`，保留最近 50 份。这保证任何改动（UI / 技能 / CLI）都可回滚。

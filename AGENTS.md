...
- **task → Plan Completed 自动转换**：Executing 期间模型用 `task_create` 拆 todos + `task_update` 推进，[`tools/task.rs::tool_task_update`](crates/ha-core/src/tools/task.rs) 在切到 `completed` 且全部 task 都终态时自动调 `maybe_complete_plan`：清理 git checkpoint + 转 `PlanModeState::Completed` + emit `plan_mode_changed { reason: "all_tasks_completed" }`。这条路径替代了已删的 `update_plan_step` 自动收尾路径，让 task 成为 plan 完成度的唯一信号源
- **git checkpoint**：审批转 Executing 时建 git checkpoint（branch/stash），让用户能 reset 到执行前。`Completed` / `Off` 时清理。这是 Hope 比 claude-code 多的能力，保留

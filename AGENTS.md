...
基于 Tauri 2 + React 19 + Rust 的本地 AI 助手桌面应用，内置丰富的 Provider 模板与预设模型，GUI 傻瓜式配置。支持三种运行模式：桌面 GUI（Tauri）、HTTP/WS 守护进程（`opencomputer server`）、ACP stdio（`opencomputer acp`）。

...
- **kernel 的 `sessions.db` 写连接不对特征 crate 开放（红线）**：`SessionDB::with_conn_internal` 恒 `pub(crate)`，特征 crate 一律走类型化方法——只读聚合自开 `SQLITE_OPEN_READ_ONLY` 连接（ha-dash），要写就说明该模块还没到能上浮的时候。**拿到裸句柄即可绕过 kernel 对 `sessions`/`messages` 的不变量与事务边界**，这条不因拆分进度让步，已两次决定分刀边界：`coding_improvement`/`domain_eval`/`domain_quality` 共 100 处 `self.conn.lock()`，因此**留 kernel**、等 typed repository 边界设计好再切，禁止用通用 `with_conn` 当过渡；`knowledge/registry.rs` 81 处（`session_db.conn.lock()` 多行写法，单行 grep 抓不到）同理**留 kernel**——机器上浮 ha-knowledge，台账不动。测试造 fixture 走 `with_conn_for_test`（`cfg(any(test, feature = "test-support"))`，生产构建里不存在）

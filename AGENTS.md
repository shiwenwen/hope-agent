...
- **工具调用前说明**：`AppConfig.tool_call_narration_enabled`（默认 `true`，可在「设置 → 聊天 → 工具调用前说明」关闭）注入 `TOOL_CALL_NARRATION_GUIDANCE`，要求模型每次工具调用前用一句话说明意图、turn 结束时简短小结。默认开是因为 IM Channel 用户没有工具调用 UI，没解说像机器人卡死；GPT-5.4 这类容易过度复读的模型可在设置面板关闭。`#[serde(default = "default_tool_call_narration_enabled")]` 让老用户升级后 config.json 缺字段也落 `true`
...
### Plan Mode（设计契约 + Task 进度双轨）

详见 [`docs/architecture/plan-mode.md`](docs/architecture/plan-mode.md)。核心约定：

- **5 状态机**：`Off / Planning / Review / Executing / Completed`（[`crates/ha-core/src/plan/types.rs`](crates/ha-core/src/plan/types.rs)）。**没有 Paused 状态**——长时间挂起就 `/plan exit` 退出，需要时 re-entry；想"暂停"就停止发消息
- **Re-entry 合法**：`Executing → Planning` / `Completed → Planning` 转移有效，让用户在执行/完成后能重进 Plan Mode 修订计划。`Off → 任意` 也始终允许
- **plan = 设计契约**：`submit_plan` 写入 `~/.hope-agent/plans/<agent_id>/<session_id>/plan-{ts}-{nano}.md`，按 agent + session 双层子目录物理隔离——模型 ls 自己的 session 目录只看到自己的 plan 文件，跨 session 误读路径堵死。老 flat 文件由启动时 [`plan::migrate_flat_plans_to_subdirs`](crates/ha-core/src/plan/file_io.rs)（spawn_blocking）按 short_id 反查 SessionDB 一次性迁移，多重/未知匹配留 flat + warn 等人工核对；幂等可重复跑。**自由 markdown，不带 checkbox / 无 step status 字段**。推荐结构 Context / Approach / Files / Reuse / Verification（推荐而非强制）。一旦审批通过即冻结，要改方案重进 Plan Mode（删了 `amend_plan` 工具，**不在 Executing 期改 plan 文件**）
- **task = 唯一进度真相**：`task_create / task_update / task_list`（[`tools/task.rs`](crates/ha-core/src/tools/task.rs)）实时进度，`pending / in_progress / completed` 三态 + `content / activeForm` 双 form。Plan 文件不再追踪 step level 进度（删了 `update_plan_step` 工具 + `PlanStep` / `PlanStepStatus` / `parser.rs`）
- **进入入口的设计契约（用户主权）**：plan mode 进入**永远由用户拍板**——绝不让模型自己转 state。三条入口都遵循这个原则：
  - **用户主动**（同步立即生效）：UI 工具栏 Plan 按钮 / `/plan enter` 斜杠 / 前端 `set_plan_mode` Tauri 命令 / HTTP `POST /api/plan/{sid}/mode`——这些是**用户已经表达意图**的入口，直接转 state
  - **模型建议（须用户审批）**：`enter_plan_mode` 工具（[`tools/enter_plan_mode.rs`](crates/ha-core/src/tools/enter_plan_mode.rs)）——模型识别非 trivial 任务时调用工具，**工具内部复用 `ask_user_question` 底层基础设施触发 Yes/No dialog**（标准 ask user UI，"Enter Plan Mode? Skip planning?" 二选一），用户接受才 `set_plan_state(Planning) + emit plan_mode_changed`，用户拒绝就 tool result 返回"User declined plan mode, continue directly"让模型继续直接做。**模型不能自己转 state**——这跟 claude-code 的 EnterPlanMode 工具设计完全对齐
  - **工具可见性**：`enter_plan_mode` 在 [`apply_plan_tools` Off 分支注入](crates/ha-core/src/agent/mod.rs)（普通会话可见），PlanAgent / ExecutingAgent 状态下不注入（已经在 plan mode 不需要"进入"工具）；`CoreSubclass::PlanMode` 默认被 dispatcher 隐藏的语义保留
  - 工具描述明确覆盖 5 类场景：Programming / Writing / Research / Information organization / Decision support，让通用任务用户也受益
- **Re-entry guidance**：`PLAN_MODE_SYSTEM_PROMPT` 顶部 Re-entry Check 段强制模型进 plan mode 后第一件事检查老 plan 文件，按"同任务增量修订 / 不同任务覆盖"分支处理
- **Executing system prompt**：[`PLAN_EXECUTING_SYSTEM_PROMPT_PREFIX`](crates/ha-core/src/plan/constants.rs) 在审批通过后注入，引导模型用 task_create 拆 todos + task_update 推进，**禁止在执行期编辑 plan 文件**
- **PlanPanel 单一职责**：右侧 PlanPanel 只渲染 plan markdown（设计契约视图）；task 进度走 [`tasks/TaskBlock.tsx`](src/components/chat/message/TaskBlock.tsx)（消息流历史）+ [`tasks/TaskProgressPanel.tsx`](src/components/chat/tasks/TaskProgressPanel.tsx)（输入框上方实时面板）。三者各司其职零重叠
- **task → Plan Completed 自动转换**：Executing 期间模型用 `task_create` 拆 todos + `task_update` 推进，[`tools/task.rs::tool_task_update`](crates/ha-core/src/tools/task.rs) 在切到 `completed` 且当前 plan 期 task 全部终态时自动调 `maybe_complete_plan`：清理 git checkpoint + 转 `PlanModeState::Completed` + emit `plan_mode_changed { reason: "all_tasks_completed" }`。**判定按 plan-期 task 范围而非整 session**——`PlanMeta.executing_started_at`（[`plan/types.rs`](crates/ha-core/src/plan/types.rs) 由 `transition_state` 转入 Executing 时 stamp）作为切片起点，过滤 `task.created_at >= start`，避免遗留 pending task 阻塞自动收尾或单纯完成旧 task 误触发；无 stamp（崩溃恢复）回退到全 session 检查避免死锁
- **git checkpoint**：审批转 Executing 时建 git checkpoint（branch/stash），让用户能 reset 到执行前。`Completed` / `Off` 时清理；**`Completed` 路径除了 `cleanup_checkpoint` 删 git ref 外还显式清 `meta.checkpoint_ref = None`**（`set_plan_state` Off 分支会 remove 整个 PlanMeta 不需要，Completed 保留 PlanMeta 必须手清，否则 `get_plan_checkpoint` 返回 stale ref → 前端 Rollback 按钮可点但 git ref 不存在）。这是 Hope 比 claude-code 多的能力
- **Plan 执行层权限强制（含 mid-turn 防御）**：`ToolExecContext.plan_mode_allowed_tools` 是 turn 起始的快照，由 [`AssistantAgent.plan_agent_mode`](crates/ha-core/src/agent/mod.rs) 字段决定。**模型在 turn 中途调 `enter_plan_mode` 切到 Planning 后**，当前 turn 剩余的 tool_call schema 仍是 Off 时的全集，模型理论上能调 write/edit/apply_patch/canvas。`resolve_tool_permission`（[`tools/execution.rs`](crates/ha-core/src/tools/execution.rs)）入口加 live state fallback：如果 ctx 快照是空 + 实时 plan_state ∈ {Planning, Review} + tool ∈ `PLAN_MODE_DENIED_TOOLS`，直接 `Decision::Deny`。这是 user-sovereignty 契约的兜底——下一条 user 消息触发 agent 重建后走标准 PlanAgent 路径
...
- **React Compiler 优先**：前端已启用 React Compiler，默认依赖编译器自动优化组件与 hooks。不要为了“防止重渲染”手动添加 `useMemo`、`useCallback` 或 `React.memo`；只有在有明确语义需求、第三方库依赖稳定引用，或 profiling 证明 compiler 无法覆盖时才允许使用，并在代码中保留简短说明。

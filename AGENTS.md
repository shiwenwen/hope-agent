...
- **plan = 设计契约**：`submit_plan` 写入 `~/.hope-agent/plans/plan-{short_id}-{ts}.md`，**自由 markdown，不带 checkbox / 无 step status 字段**。推荐结构 Context / Approach / Files / Reuse / Verification（推荐而非强制）。一旦审批通过即冻结，要改方案重进 Plan Mode（删了 `amend_plan` 工具，**不在 Executing 期改 plan 文件**）

...
- **桌面窗口透明度**：`AppConfig.window_opacity`（默认 1.0，`clamp_window_opacity` 强制 0.3–1.0）驱动 CSS 变量 `--window-opacity`，`body` 和工具类 `.bg-app-window` 通过 `color-mix(in srgb, var(--color-background) calc(var(--window-opacity)*100%), transparent)` 实现主窗口半透明。依托 `tauri.conf.json` 里已开启的 `transparent: true` + `macOSPrivateApi: true`。前端 Hook [`src/hooks/useWindowOpacity.ts`](src/hooks/useWindowOpacity.ts) 提供 `initWindowOpacity` 启动时应用值、`useWindowOpacity` 订阅 `config:changed` 热更新；设置入口 `WindowOpacitySection` 用 `isTauriMode()` 守卫，服务端/Web 模式不渲染

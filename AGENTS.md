...
- **进度走 EventBus**：拉取 `/api/pull` NDJSON 流转成 `PullProgress` 通过 `local_llm:pull_progress` 事件推送；安装脚本逐行通过 `local_llm:install_progress`。Tauri 桥接自动转发到 webview，前端用 `transport.listen` 订阅，**不**走 Tauri Channel
- **Provider 写入 contract**：通过 `mutate_config(("providers.add", "local-llm-wizard"), ...)` upsert（按 `127.0.0.1:11434` base_url 去重），随后 `mutate_config(("active_model", "local-llm-wizard"), ...)` 切活。Tauri 命令额外重建 `state.agent` 让聊天即时生效；HTTP 端依赖前端 reload provider 列表
...
**所有 code review（`/simplify`、人工审查、subagent review 等）识别出来但本期 PR 决定不修的问题，必须登记到 [`docs/plans/review-followups.md`](docs/plans/review-followups.md)，与 PR 同 commit。**

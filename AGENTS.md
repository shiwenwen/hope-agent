...
npm run sync:version   # 以 package.json 为单一来源，同步 src-tauri 版本号
npm run release:verify # 校验 package.json / src-tauri 版本一致；可附 -- --tag vX.Y.Z
...
- **桌面自动更新 / Release 版本单一来源**：`package.json` 是桌面版本的单一真相源；`npm version` 生命周期钩子会运行 `scripts/sync-version.mjs`，同步 `src-tauri/Cargo.toml` 与 `src-tauri/tauri.conf.json`。GitHub Release workflow 在 tag 构建前执行 `npm run release:verify -- --tag vX.Y.Z`，并上传 Tauri updater `latest.json` 工件。Updater 公钥跟仓库 `src-tauri/updater.pub.pem`，私钥仅存本机 `~/.tauri/hope-agent-updater.key` 和 GitHub Secrets，严禁入仓

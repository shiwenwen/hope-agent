...
- **Provider 写入 contract**：通过 `mutate_config(("providers.add", "local-llm-wizard"), ...)` upsert（按 `127.0.0.1:11434` base_url 去重），随后 `mutate_config(("active_model", "local-llm-wizard"), ...)` 切活。HTTP 端依赖前端 reload provider 列表

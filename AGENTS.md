...
- **完整能力评测只在本地显式运行**：Coding / Domain / Dreaming / Memory Retrieval 整包走独立 `hope-agent-eval`；普通 PR、pre-push、GitHub Actions 和发版 workflow 均不运行专项评测。默认 Cargo test 只保留快速契约测试。详见 [`capability-eval.md`](docs/architecture/capability-eval.md)
- **GitHub CI 不运行评测 smoke**：Rust workflow 只构建/运行 `ha-core` + `ha-server` 的普通契约测试，不构建 `ha-eval`，也不运行 fake Provider 黑盒评测。需要验证评测基础设施时，在本地显式运行 `model smoke` / `model app-smoke`
- **真实模型 Campaign 仅限本地 App / CLI**：`hope-agent-eval model` 只有显式本地运行才调用 Provider；Dashboard“能力评测”可通过 Sidecar 显式运行 Hope Core Quick/Standard/Reliability/Custom，Coding / Domain 原入口与历史继续兼容。App 只传模型/凭据引用，owner 后端解析 Key；已登录 Codex 仅在 App 本机 diagnostic 可选，owner 必须先确保 token 剩余寿命覆盖 `maxWallSeconds + margin`（不足则 owner 内刷新、失败 fail-fast），只向隔离进程传短期 access token + account id + expires-at，禁止传 OAuth 文件/refresh token。当前不配置受保护 Runner、GitHub Provider secrets、自动 Campaign、签名发布证据或评测发布门禁；本地结果只用于诊断、回归和人工发版判断。详见 [`live-model-evaluation.md`](docs/architecture/live-model-evaluation.md)
...
- **评测 GitHub workflow 暂停**：仓库当前不包含 `capability-eval.yml` / `model-campaign.yml`，`release.yml` 不查询、校验或附加 eval evidence。恢复远端评测必须单独设计并经配置 PR 显式启用，不能仅把旧 workflow 文件放回
- **本地真实模型安全边界**：隔离 `config.json` 禁止保存 Provider Key；Server/Supervisor token 必须独立，只允许合成/授权脱敏数据，禁止个人生产账号和真实用户数据。Sidecar/Supervisor 必须回收完整后代树；本地签名 bundle 与导入信任仍可用于诊断，但不构成 GitHub 发布证据。Headless Server 只接受显式绝对 `HA_EVAL_TRUST_REGISTRY_PATH`，禁止 cwd/exe ancestor 搜索
...
- **桌面 release 单一来源**：`package.json`，`pnpm version` 钩子（[`scripts/sync-version.mjs`](scripts/sync-version.mjs)）同步 `src-tauri/Cargo.toml` / `tauri.conf.json` / `crates/ha-server/Cargo.toml` / `crates/ha-core/Cargo.toml` / `crates/ha-eval/Cargo.toml`。`ha-server` 承载 Docker headless bin `hope-agent-server` 的 `CARGO_PKG_VERSION`，必须随桌面版本同步——`--version` 与 `app_update` `current_version` 都读它；`ha-core` 与 `ha-eval` 跟随产品版本，确保 App、Server 与本地评测 Sidecar 协议一致。CI tag 构建前跑 `pnpm release:verify -- --tag vX.Y.Z` 校验全部来源 + `Cargo.lock` 一致。Updater 私钥严禁入仓
...
- **专项评测版本与网络边界**：`evals/version-lock.json` 已有 `id@version` 只许保留原 digest，新内容必须提升版本并追加 key；当前通过本地 `node scripts/verify-eval-version-lock.mjs --base <base-sha>` 与代码审查守护，不在 GitHub CI 执行。App 晋升 `eval_candidate` 必须同时登记 manifest、提升 suite patch version 并追加 lock，任一步失败不得标记 `promoted`。本地 `networkPolicy=deny` 只有在真实 OS sandbox / network namespace 中才成立，环境变量不能冒充隔离
- **真实模型评测边界**：`evals/live/version-lock.json` 同样 append-only；manifest 只允许注册 adapter / verifier / fault 枚举，禁止 shell。`EvalRunContext` 只在 `HA_MODEL_EVAL_MODE=1` 接受，新增 Goal / Workflow / Async / Agent 执行边界必须传播身份并关闭 guard。当前只支持本地 App / CLI 运行；未来若恢复专用 Runner，Provider-only 外部防火墙仍必须作为真实网络边界，环境变量只能作为部署证明

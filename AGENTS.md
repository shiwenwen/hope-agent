...
- **Session 模式三选一**：`sessions.permission_mode ∈ default | smart | yolo`，标题栏 `PermissionModeSwitcher` dropdown 切换。`AgentConfig.capabilities.default_session_permission_mode` 决定该 Agent 新会话的初始 mode（None = 跟随全局默认 `default`）
...
- `AppConfig.default_agent_id` 默认 `Some("default")`，可在「设置 → 常规 → 系统」里改；`oc-settings` 工具支持 `category="default_agent"` 读写（LOW 风险）。Tauri 命令 `get_default_agent_id` / `set_default_agent_id`，HTTP `GET/PUT /api/config/default-agent`。
...
- **MEDIUM**：行为调整，影响上下文、成本或输出质量（compact / memory\_\* / web_search / approval / smart_mode 等）
- **HIGH**：安全、网络暴露、全局键位、凭据、需要重启、**权限规则改动**（proxy / embedding / shortcuts / server / skill_env / acp_control / `permission.global_yolo` / `protected_paths` / `dangerous_commands` 等）——技能在 `update_settings` 前必须向用户二次确认。`global_yolo` 跨整 process 影响所有会话；`protected_paths` / `dangerous_commands` 写错或删除条目会让本不该放行的命令直接走 Allow，必须二次确认

...
- **execute_with_failover executor（chat_engine 主路径接入）**：`crates/oc-core/src/failover/executor.rs` 提供泛型 `execute_with_failover<T, F: FnMut(Option<&AuthProfile>) -> Fut>(provider, session_id, policy, on_profile_rotation, op)` 把"profile 选择 + 调 op + 错误分类 + 决策"四步从 `chat_engine::engine` 内 hand-rolled loop 抽出来。`FailoverPolicy` 三档默认值（`chat_engine_default` / `side_query_default` / `summarize_default`）覆盖不同重试 + 轮换策略。`ExecutorError` 三种终态：`NeedsCompaction { last_profile }`（外层接住后做 emergency_compact + 把 last_profile 写回 `PROFILE_STICKY` 让 retry 命中同 key 不破坏 prompt cache）/ `Exhausted { last_reason, last_error }` / `NoProfileAvailable`。Codex defense-in-depth：即使 caller 传 `allow_profile_rotation: true`，`provider.api_type == Codex` 时强制 `false`。chat_engine 接入后内层 hand-rolled loop 完全消失，剩 outer compaction-retry loop + 单次 executor 调用 + ChatRoundOk 中转 struct。**`side_query` / `summarize_direct` 接入 executor 暂未实施**（需要 `AssistantAgent` 持有 `provider_config_id` 字段以反查 `ProviderConfig` + 处理 `OneShotMode` lifetime capture，侧路径低优先级，留 follow-up PR）

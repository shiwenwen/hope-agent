...
- **定时任务支持把结果推送到 IM 渠道**：`CronJob` 新增 `delivery_targets: Vec<CronDeliveryTarget>` 字段，每个 job 可 fan-out 到任意数量的 IM 会话（Telegram / 飞书 / Slack / Discord / WeChat / ...）。成功时发送 assistant 最终文本，失败时发送 `⚠️ [Cron] {name} failed: {error}` 简短错误。前端在 Cron 任务编辑表单新增"Send result to IM channel"区块（账户 + 会话两级下拉 + 多目标 + 删除），后端新建 [`crates/oc-core/src/cron/delivery.rs`](crates/oc-core/src/cron/delivery.rs) 复用 `ChannelRegistry::send_reply` 统一入口，10s/target 超时保护避免单个 channel hang 卡住 scheduler。`manage_cron` 工具扩展两个新 action — `list_channel_targets`（枚举可用账户 + 会话供模型拾取）和 `update`（字段增量覆盖）— 让模型能通过自然语言管理定时任务的发送目标；当用户在 IM 渠道会话中用自然语言创建 cron 时（且未显式指定 `delivery_targets`），后端会反查 `ChannelDB.get_conversation_by_session(session_id)` 自动推断"发回当前这个 chat"为默认目标，显式传 `delivery_targets=[]` 可关闭

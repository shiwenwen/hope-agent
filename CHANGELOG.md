...
- **桌面窗口透明度设置**：新增 `AppConfig.window_opacity`（默认 `1.0`，范围 0.3–1.0），可通过设置 → 通用 → 外观与语言 的新卡片"窗口透明度"滑块实时调节。利用 Tauri `transparent: true` + `macOSPrivateApi: true` 的已有能力，在 CSS 层通过 `--window-opacity` 变量 + `color-mix()` 作用在 `body` 和新工具类 `.bg-app-window` 上，让桌面合成器透过主窗口显示底层桌面。仅桌面模式（`isTauriMode()`）渲染该设置入口，服务端持久化但无视觉效果。新增 Tauri 命令 `get_window_opacity` / `set_window_opacity`、HTTP 路由 `GET|POST /api/config/window-opacity`、`oc-settings` 技能 `window_opacity` 分类（LOW 风险，字段 `windowOpacity`）。前端 Hook `useWindowOpacity` 在启动时从 config 读取并 set CSS 变量，同时订阅 `config:changed` 以便技能改动也能热更新；i18n 新增 zh + en 三条文案

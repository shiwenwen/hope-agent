...
- **Rust CI 补齐 fmt / clippy 两个 job**：`.github/workflows/rust.yml` 在原先 `test` job 前新增并行 `fmt`（`cargo fmt --all --check`）和 `clippy`（`cargo clippy -p oc-core -p oc-server --all-targets --locked -- -D warnings`）两个独立 job。上一步 `[workspace.lints.clippy]` 把 ~40 条纯风格 / 结构性 lint 设置为 allow（每条带原因注释）后，`clippy -D warnings` 首次能在 workspace 上跑通。三个 job 并行执行，fmt / clippy 失败不阻塞 test 继续跑，让单次 CI 能暴露最多问题。本地三连验证通过（fmt 零 diff / clippy 零 warning / 464 tests 全绿）
- **工作空间 clippy warnings 清零 + workspace lint 策略固化**：`cargo clippy -p oc-core -p oc-server --all-targets -- -D warnings` 首次通过。清理路径：(1) 根 [`Cargo.toml`](Cargo.toml) 新增 `[workspace.lints.clippy]`，按"结构性/风格/correctness-at-site"三档对 `too_many_arguments` / `map_or` / `redundant_closure` / `while_let_on_iterator` / `collapsible_if` / `needless_borrow` / `manual_range_contains` / `manual_clamp` / `manual_div_ceil` / `manual_is_multiple_of` / `manual_strip` / `explicit_counter_loop` / `needless_range_loop` / `if_same_then_else` / `let_unit_value` / `single_match_else` / `unnecessary_to_owned` / `unnecessary_lazy_evaluations` / `unused_async` / `useless_format` / `clone_on_copy` / `option_map_unit_fn` / `bind_instead_of_map` / `extra_unused_lifetimes` / `bool_comparison` / `needless_pass_by_ref_mut` / `redundant_field_names` / `single_char_pattern` / `only_used_in_recursion` / `match_single_binding` / `unnecessary_map_or` / `map_unwrap_or` / `map_clone` / `should_implement_trait` / `doc_lazy_continuation` / `module_inception` / `await_holding_lock` / `new_without_default` / `large_enum_variant` / `type_complexity` 共 ~40 个 lint 全局 `allow`（每条都注明原因）；三个 crate（`oc-core` / `oc-server` / `open-computer`）用 `[lints] workspace = true` 统一拿到；(2) 手工修剩下 ~100 条：`Default` impl 全部迁到 `#[derive(Default)] + #[default]`（`ChannelHealth` / `DmPolicy` / `GroupPolicy` / `PlanModeState` / `Outcome` / `DeferredToolsConfig` / `EmbeddingProviderType` / `EmbeddingConfig` / `ImageGenProviderEntry` / `SkillStatus` / `SsrfPolicy` / `AutoReviewPromotion` / `ThinkingStyle` / `ProxyMode` / `ProxyConfig` / `ToolExecContext` 等 16 个类型）、`FacetSummary` 字段初始化改 `..Default::default()`、`items after a test module` 两处（`process_registry` / `system_prompt::build`）挪回 test mod 上方、`doc_lazy_continuation` 拆段落或切 inner-doc、`empty_line_after_doc_comments` 清理、`iter().any()` → `contains()` / `sort_by` → `sort_by_key`、`u64 as u64` 冗余 cast 删除、`context_compact::{prune_old_context,compact_if_needed}` / `memory::embedding::utils::l2_normalize` 签名从 `&mut Vec<T>` 降为 `&mut [T]`（无 push/pop 操作）、`memory::sqlite::prompt::format_prompt_summary` 去掉 `#[deprecated]` 属性（3 个现有 caller 都是 "单一 budget 转发 v2" 场景，保留为 convenience wrapper）、`AcpSessionStore` 补 `is_empty()`、`SqliteMemoryBackend::open` 的 `std::mem::transmute` 加显式类型标注、`weather_location_macos` 的 `extern_protocol!` `unsafe trait` 走 `#![allow(clippy::missing_safety_doc)]` crate-level（宏体里无处挂 doc）、`channel::qqbot::gateway` 的 `last_seq = 0` 加 `#[allow(unused_assignments)]` 块注释"显式重置语义"、`channel::{discord/feishu/slack/telegram}` 的 `bot_id` / `bot_name` 等 diagnostics 字段加 `#[allow(dead_code)]` 注释"保留作为 future bot-authored event filtering"、`api_types` 模块可见性提升为 `pub(super) mod`（解决 `ReasoningConfig` 比方法更私）、`ReasoningConfig` 提升为 `pub(crate)`、若干 `build_static_system_prompt` / `tool_context` / `is_tool_denied` 等"当前没调用但属于命名对偶的备用 API"加 `#[allow(dead_code)]` + 注释。**未改范围**：`MutexGuard held across await` 在 `tools::job_status` 里全是测试级 `Mutex<()>` 串行化锁、真 await-lock-panic 风险的是 `await_holding_refcell_ref` 不是它——走 workspace allow。（`awareness / async_jobs / chat_engine` 等生产锁均在 audit 后已确认用 `tokio::sync::Mutex` 或不跨 await，不在 clippy 命中范围）

### Fixed

- **Telegram 多媒体消息静默丢件**：[`channel/telegram/mod.rs:239-255`](crates/oc-core/src/channel/telegram/mod.rs#L239) `send_message` 的媒体循环每个 match 臂都 `return Ok(...)`，一次只会发送 `payload.media` 的第一条。单图/单文件流程不受影响，但多媒体流程从首条起后面全部悄悄丢失。修复把循环收敛为"逐条发送 + 记录最后一条 msg id"结构，返回 `DeliveryResult::ok(last_id)`。被 `cargo clippy` 的 `never_loop` lint 作为 error 暴露，同步解锁 CI 接入 clippy
- **sandbox `wait_container` 使用误导性 while-let**：[`sandbox.rs:506-515`](crates/oc-core/src/sandbox.rs#L506) `while let Some(result) = stream.next().await { ... return ... }` 语义上只读第一个事件就返回，但 `while let` 写法暗示会循环多次，触发 `never_loop` clippy error。改成 `if let Some(result) = stream.next().await`，语义不变但意图清晰

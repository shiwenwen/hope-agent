...
```bash
# Run specific failing test (Python example)
pytest tests/test_module.py::test_name -v

# Rust example
cargo test -p my-crate failing_test_name -- --nocapture
...
- Ask the user for help (use `ask_user_question` tool)
- Research more
...
Use these Hope Agent tools during Phase 1:

- **`grep`** — Find error strings, trace function calls, locate patterns (regex over file tree)
...
- **`ask_user_question`** — Pause to gather more information when stuck

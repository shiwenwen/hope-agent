...
Hope Agent convention — project cwd gets `.hope-agent/skills/<name>/`,
standalone invocation writes `~/.hope-agent/skills/<name>/`.
...
def default_skill_root() -> Path:
    ...
    marker — when found, use `<repo>/.hope-agent/skills/`. Otherwise fall
    back to `~/.hope-agent/skills/` so the skill lives in the user-level
    ...
    for candidate in [cwd, *cwd.parents]:
        if (
            ...
            return candidate / ".hope-agent" / "skills"
    return Path.home() / ".hope-agent" / "skills"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scaffold a new Hope Agent skill.",
    ...
    parser.add_argument(
        ...
        "Default: .hope-agent/skills in the nearest repo, else ~/.hope-agent/skills.",
    )
    parser.add_argument(
        "--resources",
        default="",

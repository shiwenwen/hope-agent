...
| `async_tools` | `enabled`, `autoBackgroundSecs`, `maxJobSecs`, `maxConcurrentJobs` (default: hardware-derived `clamp(logical_cores − 2, 4, 16)`, `0` = unlimited; cap on concurrent `run_in_background` jobs — each holds an OS thread; at the cap a new background request returns an error result telling the model to wait/poll `job_status` or run synchronously), `completionMergeWindowSecs` (u64, default `3`, `0` = disabled; R4: when several background jobs in the same session finish within this window, their completion notifications merge into ONE injected turn instead of N billed turns — `Group` is the pre-merged special case), `inlineResultBytes`, `retentionSecs`, `orphanGraceSecs`, `jobStatusMaxWaitSecs` |

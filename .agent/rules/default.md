...
- **会话持久化**：`session.rs` 基于 SQLite（WAL 模式）持久化会话及消息。`SessionDB` 管理 sessions 和 messages 两张表，支持 user/assistant/system/tool 四种消息角色。`chat` 命令自动创建/关联会话，保存用户消息、助手回复、工具调用结果和降级事件（`role=system`）。`paths.rs` 提供 `attachments_dir()` 管理附件存储。新增 Tauri 命令：`create_session_cmd` / `list_sessions_cmd` / `load_session_messages_cmd` / `delete_session_cmd`

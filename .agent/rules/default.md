...
- **画布工具**：`tools/canvas/` 实现交互式可视化内容创作工具。统一 `canvas` 工具，11 个 action，7 种内容类型。前端 `CanvasPanel.tsx` iframe 沙箱渲染，嵌入 ChatScreen 右侧。配置在 `config.json` 的 `canvas` 字段
...
- **技能系统**：`skills.rs` 实现完整技能发现与管理系统。SKILL.md frontmatter 定义技能，三层目录发现，支持嵌套检测。**懒加载 Prompt**：仅注入目录（名称+描述+路径），LLM 按需 read。**三层预算降级**：Full → Compact → 二分截断。**Requirements 增强**：bins(AND) + anyBins(OR) + env + os + config + always + primaryEnv。**调用策略**：user-invocable / disable-model-invocation。**安装引导**：brew/node/go/uv/download。**健康检查** + **缓存** + **Bundled Allowlist**
- **斜杠命令系统**：`slash_commands/` 模块实现 channel-agnostic 命令系统，16 个内置命令分 6 类（Session/Model/Memory/Agent/Utility/Skill）。**Skill 命令动态注册**：user-invocable 的技能自动注册为斜杠命令。支持 command-dispatch 绑定工具。后端 `registry.rs` 声明式命令注册表，`parser.rs` 文本解析，`handlers/` 按类别拆分，dispatch 模式分发执行。3 个 Tauri 命令（`list_slash_commands` / `execute_slash_command` / `is_slash_command`），返回 `CommandResult`（content + `CommandAction` 枚举）。前端 `SlashCommandMenu.tsx` 弹出菜单 + `useSlashCommands.ts` hook + `ChatInput.tsx` "/" 按钮和键盘拦截

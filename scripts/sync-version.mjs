...
try {
  execSync(
    "cargo update -p hope-agent -p ha-server -p ha-core -p ha-acp -p ha-browser -p ha-design -p ha-mac -p ha-updater -p ha-vcs -p ha-weather -p ha-browser-host -p ha-eval --offline --quiet",
    {
      cwd: rootDir,
      stdio: "inherit",
    },
  ...
  console.error(
    "[sync-version] failed to sync Cargo.lock; ensure Rust toolchain is installed, or run `cargo update -p hope-agent -p ha-server -p ha-core -p ha-acp -p ha-browser -p ha-design -p ha-mac -p ha-updater -p ha-vcs -p ha-weather -p ha-browser-host -p ha-eval` manually",
  )
  process.exit(1)
}
...
if (process.env.npm_lifecycle_event === "version") {
  try {
    ...
    execSync(
      "git add package.json src-tauri/Cargo.toml src-tauri/tauri.conf.json crates/ha-server/Cargo.toml crates/ha-core/Cargo.toml crates/ha-base/Cargo.toml crates/ha-config-schema/Cargo.toml crates/ha-browser-host/Cargo.toml crates/ha-eval/Cargo.toml crates/ha-updater/Cargo.toml crates/ha-vcs/Cargo.toml crates/ha-weather/Cargo.toml crates/ha-acp/Cargo.toml crates/ha-mac/Cargo.toml crates/ha-design/Cargo.toml crates/ha-browser/Cargo.toml Cargo.lock",
      {
        cwd: rootDir,
        stdio: "ignore",
      },

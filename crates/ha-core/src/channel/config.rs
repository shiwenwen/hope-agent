...
// 类型已下沉 ha-config-schema：`ChannelStoreConfig`（含 inherent impl）
// 原地再导出，`crate::channel::config::ChannelStoreConfig` 路径不变。
pub use ha_config_schema::channel::ChannelStoreConfig;

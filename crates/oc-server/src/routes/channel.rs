...
) -> Result<Json<oc_core::channel::wechat::login::WeChatLoginStart>, AppError> {
    Ok(Json(
        oc_core::channel::wechat::login::start_login(body.account_id.as_deref()).await?,
    ))
}
...
) -> Result<Json<oc_core::channel::wechat::login::WeChatLoginWait>, AppError> {
    Ok(Json(
        oc_core::channel::wechat::login::wait_login(&body.session_key, body.timeout_ms).await?,
    ))
}

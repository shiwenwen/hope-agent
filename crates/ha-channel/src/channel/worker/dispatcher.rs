...
async fn handle_inbound_message(
    ...
    if ha_core::slash_defs::parser::is_command(user_text) {
        ...
        .await
        {
            Ok(ChannelSlashOutcome::Reply {
                ...
                if new_session_id.is_none() {
                    if let Err(e) = ha_core::slash_defs::append_slash_history_events(
                        &session_db,
                        effective_sid,
                        user_text,
                        Some(&content),
    ...
    .await
    {
        ...
        ha_core::agent::preflight::PreflightOutcome::Block { reason } => {
            ...
            let _ = session_db
                .append_message(&session_id, &ha_core::session::NewMessage::event(&notice));
            let _ = plugin
    ...
    let user_attachments_meta = match ha_core::attachments::persist_chat_user_attachments_meta(
        &session_id,
        &mut attachments,
    ) {
        Ok(meta) => meta,
    ...
    let persist_result = session_db
        .run(move |db| -> anyhow::Result<()> {
            ...
            let _ = ha_core::session::ensure_first_message_title(
                db,
                &persist_session_id,
                &title_prompt,
    ...
    let cancel = match ha_core::globals::get_channel_cancels() {
    ...
    impl Drop for CancelHandleGuard {
        fn drop(&mut self) {
            if let Some(reg) = ha_core::globals::get_channel_cancels() {
                reg.remove(&self.0);
            }
        }
    }

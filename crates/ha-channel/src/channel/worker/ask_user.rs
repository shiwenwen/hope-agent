...
fn ask_user_timeout_notice(
    ...
    let template = if used_default_values {
        tr(
            locale,
            [
                "⏱ 问题 `#{tag}` 已在 {secs} 秒后超时。默认方案已交给 AI 参考，不代表你的确认。{question}",
                "⏱ 問題 `#{tag}` 已在 {secs} 秒後逾時。預設方案已交給 AI 參考，不代表你的確認。{question}",
                "⏱ Question #{tag} timed out after {secs}s. Defaults were returned to AI as assumptions, not your confirmation.{question}",
                "⏱ 質問 `#{tag}` は {secs} 秒後にタイムアウトしました。既定案を仮定として AI に渡しました。あなたの承認ではありません。{question}",
                "⏱ 질문 `#{tag}`가 {secs}초 후 만료되었습니다. 기본 방안을 가정으로 AI에 전달했으며 사용자의 확인이 아닙니다.{question}",
                "⏱ La pregunta `#{tag}` caducó tras {secs}s. Las opciones predeterminadas se enviaron a la IA como supuestos, no como tu confirmación.{question}",
                "⏱ A pergunta `#{tag}` expirou após {secs}s. As opções padrão foram enviadas à IA como suposições, não como sua confirmação.{question}",
                "⏱ Вопрос `#{tag}` истек через {secs} с. Варианты по умолчанию переданы ИИ как предположения, а не ваше подтверждение.{question}",
                "⏱ انتهت مهلة السؤال `#{tag}` بعد {secs} ثانية. أُرسلت الخيارات الافتراضية إلى الذكاء الاصطناعي كافتراضات، وليس كتأكيد منك.{question}",
                "⏱ `#{tag}` sorusu {secs} sn sonra zaman aşımına uğradı. Varsayılan seçenekler yapay zekaya varsayım olarak iletildi; onayınız değildir.{question}",
                "⏱ Câu hỏi `#{tag}` hết hạn sau {secs} giây. Các phương án mặc định đã được gửi cho AI dưới dạng giả định, không phải xác nhận của bạn.{question}",
                "⏱ Soalan `#{tag}` tamat masa selepas {secs}s. Pilihan lalai dihantar kepada AI sebagai andaian, bukan pengesahan anda.{question}",
            ],
        )
    } else {
        tr(

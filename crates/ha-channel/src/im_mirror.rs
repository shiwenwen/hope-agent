...
impl ha_core::channel_hooks::ImLiveMirror for ImLiveMirrorState {
    fn finalize<'a>(
        ...
        Box::pin(async move { finalize_im_live_mirror(*self, response).await })
    }
...
pub(crate) fn attach_live_hook<'a>(
    ...
    Box<
        dyn std::future::Future<Output = Option<Box<dyn ha_core::channel_hooks::ImLiveMirror>>>
            + Send
            + 'a,
    >,
> {
    Box::pin(async move {
        attach_im_live_mirror(session_id, source, last_user)
            .await
            .map(|s| Box::new(s) as Box<dyn ha_core::channel_hooks::ImLiveMirror>)
    })
}
...
> {
    Box::pin(async move {
        attach_im_injection_mirror(session_id)
            .await
            .map(|s| Box::new(s) as Box<dyn ha_core::channel_hooks::ImLiveMirror>)
    })
}

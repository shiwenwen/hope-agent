...
<tr><td><b>🦙 本地小模型一键安装</b></td><td>设置 → 模型页面自动检测电脑配置（macOS 看统一内存 / Windows · Linux 优先 dGPU 显存），按"模型 ≤ 可用资源 50%"原则推荐 Qwen3.6 / Gemma 4 系列里能跑得动的尺寸（含 Qwen3.6 27B/35B-A3B、Gemma 4 e2b/e4b/26b-A4B/31b），一键完成 Ollama 安装、模型下载、Provider 注册、active model 切换，零命令行。Windows 用户引导跳官网下载，macOS / Linux 跑官方一键脚本。</td></tr>
